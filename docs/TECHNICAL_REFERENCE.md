...
semantic scale. The exact filament binary remains lazy until that LOD despite its visible-by-default
policy. One selection point is reused for every catalogue target, every retained record remains
...
Tempel et al. (2014). The 4,533,016-byte binary is referenced by the manifest but fetched only when
the cosmic-web LOD, its filament layer, or a filament target requires it. A 64-byte header records
the reference frame, unit, epoch, dimensions, point-distance bounds, and available metrics. Each
8-byte index entry stores the source filament ID, point count, and contiguous point offset; each
16-byte point stores three `float32` J2000 Cartesian coordinates plus quantized visit-map,
weighted-density, and orientation-strength values.

This background request deliberately does not publish the engine's blocking startup-loading state:
the full-screen initialization overlay therefore cannot intercept wheel or pointer input while the
catalogue downloads or its GPU tiles are constructed. Failures publish one nonfatal performance
warning and leave every other cosmic layer navigable.
...
- all 260,178 Tempel source segments retained in typed arrays and partitioned across four non-empty
  spatial GPU line tiles, with lazy fetching, quality-aware draw ranges, and reusable whole-spine
  hover and selection geometry;
...
- deferred Tempel binary loading only at the cosmic-web scale, exact published dimensions, spatial
  tile activation, progressive segment disclosure, F1 search, and whole-spine selection;
...
1. move larger-catalogue decoding and hierarchy preparation into Web Workers, starting with the
   Tempel line asset if cross-device profiling identifies a main-thread transition hitch;
