...
test('la recherche localise Sirius dans le ciel terrestre à la date de la carte', async ({
  ...
  await expect(journey.locator('.earth-sky-view__atmosphere')).toHaveCount(1);
  ...
  await expect(horizonProfile.locator('[data-cityscape-svg]')).toHaveCount(0);
  await expect(horizonProfile.locator('.earth-sky-view__ridge')).toHaveCount(2);
...
test('les planètes restent ancrées au fond stellaire dans le ciel terrestre', async ({ page }) => {
  ...
  const initialCatalogStar = await waitForLabelCenter(page, 'hyg-65269');
  ...
  const movedCatalogStar = await waitForLabelCenter(page, 'hyg-65269');
  const moonDeltaX = movedMoon.x - stationaryMoon.x;
  const starDeltaX = movedCatalogStar.point.x - initialCatalogStar.point.x;
  ...
  const timedCatalogStar = await waitForLabelCenter(page, 'hyg-65269');
  const horizonAfterTime = await horizon.boundingBox();
  const moonTimeDeltaX = timedMoon.x - movedMoon.x;
  const starTimeDeltaX = timedCatalogStar.point.x - movedCatalogStar.point.x;
  ...
  expect(Math.abs(starTimeDeltaX)).toBeGreaterThan(20);
...
test('la Voie lactée volumique remplace l’amas artificiel sans charger ses agrégats', async ({
  page,
}) => {
  test.setTimeout(60_000);
  const browserErrors = monitorBrowserErrors(page);
