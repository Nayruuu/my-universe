...
test('le ciel terrestre masque la lumière solaire et conserve une Lune lisible', async ({
  ...
  await expect(moon).toHaveAttribute('data-renderer', 'webgl-existing-object');
  await expect(moonDisk).toHaveCSS('width', '34px');
  await expect
    .poll(() => moonDisk.evaluate((element) => getComputedStyle(element).backgroundImage))
    .toBe('none');
  await expect
    .poll(async () => (await readObjectVisualDiagnostics(page, 'moon'))?.nearVisible)
    .toBe(true);
  expect(await readObjectVisualDiagnostics(page, 'moon')).toMatchObject({
    bodyPresent: true,
    bodyVisible: true,
    visualVisible: true,
    nearVisible: true,
    surfaceTexture: {
      requested: true,
      source: expect.stringContaining('moon-lroc-'),
    },
  });
