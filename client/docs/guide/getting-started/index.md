...
Universe Map opens a separate local night-sky projection at the map's current date and selected
observation location. Drag to look around, use the wheel or a pinch gesture to change the field of
view, and select **Recenter** to recover the target. Names and modern constellation figures can be
toggled independently; the figure containing the target is brighter when it is in view.

This observer view computes direction, altitude, azimuth, precession, nutation, and atmospheric
refraction locally. It does not currently model weather, terrain, daylight visibility, or catalogue
proper motion.
