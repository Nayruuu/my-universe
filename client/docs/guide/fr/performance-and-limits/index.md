...
ceux de l’hôte ; moyenne et faible sont des proxys de régression, jamais des preuves matérielles. Un
budget dépassé n’interrompt plus les protocoles suivants : le manifeste complet inscrit
`withinBudget` pour chaque protocole, chaque profil et la campagne, puis le mode strict renvoie un
échec. `UNIVERSE_BENCHMARK_STRICT=0` ne sert qu’à collecter volontairement une baseline déjà connue
comme régressive.
...
Le panneau et le benchmark des traversées affichent aussi le préchauffage du rendu initial puis de
la scène Tempel. Chaque valeur est une durée murale monotone de préparation demandée à Three.js ;
elle aide à rapprocher un pic d’image d’un travail de compilation ou de texture, mais ne prétend pas
mesurer le temps d’exécution GPU.
