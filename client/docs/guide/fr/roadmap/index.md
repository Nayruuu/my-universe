...
_Dernière révision : 15 septembre 2026._
...
- La calibration structurelle de la Voie lactée utilise désormais la même échelle canonique pour
  ...
  traversée à l’intérieur, sans échange d’objet visuel. Les 280 000 points de haute qualité effectivement rendus
  ...
  extérieure ajoute désormais un unique lot GPU de 16 000, 32 000 ou 64 000 étoiles clairsemées
  autour du disque selon la qualité, dont un seizième forme 48 concentrations compactes évoquant des
  amas globulaires. Cette enveloppe plus ronde atteint environ 129 kpc du centre galactique, reste
  fixe dans le référentiel galactique, produit uniquement une parallaxe de perspective et s'efface
  avant le voisinage solaire. Elle est explicitement illustrative et non cataloguée, sans brouillard
  ni émission diffuse. Une passe distincte, fixe dans le référentiel galactocentrique, ajoute désormais
  6 000, 14 000 ou 28 000 silhouettes étendues de galaxies autour
  ...
  galaxies — avec une zone d’évitement galactique analytique et une parallaxe de perspective. Le
...
## Terminé le 11 septembre 2026

- Les optimisations CPU et catalogues sont livrées : préparation par lots des géométries et
  registres, caches de recherche et labels, publication complète recherche/découverte et calcul
  des ancêtres une fois par image. Le panorama 8K est décodé de façon asynchrone avec un
  préchauffage initial borné. Trajets de caméra, coordonnées et valeurs scientifiques sont inchangés.
- La campagne propre sur `796fe70` valide **10/10** budgets protocole/profil, avec trois passages
  ou cycles de ressources par cas : medium/CPU 4× et low/CPU 6×. Les médianes Tempel sont
  de 27,2 / 29,9 ms et ses six premières images restent sous 33,3 ms. Les pires images des
  traversées sont à 100,0 / 66,7 ms. Aucun écart des compteurs de ressources ; Jupiter est
  résolu 3/3 par profil. Cette campagne est terminée, sans garantir l'absence de tout pic.
  Le CPU ralenti conserve le GPU et la mémoire de l'hôte : ce n'est pas une preuve sur un autre matériel.
- Les correctifs de sécurité sont livrés dans `c380bce` : audit npm à zéro vulnérabilité le
  11 septembre et vérification complète réussie, dont 180 tests navigateur et une couverture
  de 100 %. Le 14 septembre, l’API Dependabot de GitHub ne retourne aucune alerte ouverte. La campagne de performances
  précède ces mises à jour et ne doit pas être attribuée à `c380bce`.

## Priorités actuelles

1. Valider ensemble l'équilibre visuel bras/cœur/halo et la sensation de traversée face aux
   références. La visibilité technique ne vaut pas approbation esthétique ; ne pas modifier la caméra.
2. Surveiller le premier travail de rendu et les pics. Le panneau Debug et le benchmark des
   traversées distinguent désormais les préchauffages initiaux et Tempel, sans les présenter comme
   des temps GPU. Le medium atteint la limite de 100 ms ; le pic Tempel isolé à 78,2 ms n'a pas été
   reproduit dans les diagnostics suivants, sans preuve de correction.
3. Rejouer la campagne propre après les mises à jour de dépendances avant de revendiquer une
   nouvelle référence de performances.
4. Tester sur du matériel réellement moyen/faible si disponible ; cette validation reste facultative.
