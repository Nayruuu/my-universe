...
Wheel zoom follows the screen pointer. When an inward gesture starts over a navigable label or
rendered object, that object is locked for the burst and becomes both the navigation target and the
zoom anchor. The approach remains continuous and stops in front of its contextual distance floor;
additional samples from the same burst cannot cross it. Automatic scale transitions may still
replace the target with the logical parent or child required by the map hierarchy. Over empty space,
the geometric pivot follows the pointer. Inward input from a reached target releases it only when the
pointer no longer targets that object, then camera and pivot travel at a constant distance while the
selected card remains open. Reversing the wheel retraces the recorded free-space route before
distance zoom resumes.
...
- The logical **target** drives the semantic route and contextual distance limits.
- The geometric **pivot** is the point around which the camera orbits and follows pointer zoom.
