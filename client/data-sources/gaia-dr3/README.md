...
This dataset does not replace HYG. HYG keeps the 10,000 named/searchable points, proper-motion
propagation, labels, and constellations. Gaia rows become distant anonymous spatial aggregates and
bounded samples of real catalogue sources used as a dense local field. Every retained sample keeps
its Gaia DR3 `source_id` and is selectable and focusable directly from its shared GPU batch, but is
deliberately omitted from global search and labels.
...
weighted by G flux. Refined samples retain each selected source's `source_id`, catalogue position,
G magnitude, and BP−RP value. Their integer density weights sum back to the child tile's complete
source count; those weights do not turn a sample into an aggregate position. Cell coordinates and
photometry have `calculated` confidence; sample identifiers, coordinates, and photometry are
measured catalogue values; point size, opacity, colour mapping, and every density weight are
rendering adaptations. Only sampled sources are interactive: calculated cells remain anonymous
because one cell represents many stars. The hierarchy contains 127 roots, 3,837 child tiles,
133,526 measured samples, and 135 request packs. Runtime parsing happens in module Workers where
available, and typed-array buffers are transferred to the main thread without copying. One
`THREE.Points` batch is used per active representation, never one Three.js object per source.
