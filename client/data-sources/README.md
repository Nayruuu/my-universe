...
### Calculated terrain-obstruction horizons

All 461 fixed observer locations have a compact 360° obstruction profile calculated offline from the
global [NOAA/NCEI ETOPO 2022](https://www.ncei.noaa.gov/products/etopo-global-relief-model) 60
arc-second surface-elevation grid ([DOI 10.25921/fd45-gt74](https://doi.org/10.25921/fd45-gt74)).
The source GeoTIFF is not committed or shipped. The browser lazily loads a validated manifest and a
little-endian centidegree binary only when the planetarium opens. See
[`earth-terrain-horizons/README.md`](earth-terrain-horizons/README.md) for the geometric calculation,
resolution limits, provenance, and reproducible command.
...
pattern. The absolute screen scale, horizontal spacing, skyline buildings, and Montmartre terrain
offset remain visual orientation cues rather than a surveyed Paris panorama. The layer does not
reconstruct weather or the historical appearance of the city at the selected date.
...
All 461 observer locations additionally have a static four-entry landmark pack generated at build
time. The generator searches nearby English Wikipedia pages with Earth coordinates, adds four
overlapping peripheral searches whenever the 500-page geosearch limit is reached, retains stable
Wikidata identifiers, and rejects non-physical and discontinued subjects. It ranks the remaining
records with article depth and the preceding 60 days of Wikipedia page views, then diversifies the
selected categories and bearings and retrieves a documented Wikidata `height` statement when one is
available. Sparse locations receive explicit GeoNames-based skyline anchors rather than invented
monument names. Every record preserves its source URL, selection method, scientific confidence,
height confidence, and a separate `illustrative` visual confidence.

The source snapshot lives in `data-sources/earth-landmarks/`; compact runtime files live in
`public/data/earth-landmarks/`. The browser fetches the manifest only when a procedural Earth horizon
is needed, then lazily loads and caches the matching time-zone-region pack. Only the four records for
the active city are materialized as SVG. The paths are deterministic category-based visual
silhouettes, not claims that the actual facade or surveyed skyline geometry was reconstructed.
Hand-authored city panoramas and landmark overrides remain available for Paris, New York, Tokyo,
London, Sydney, Cairo, Rio de Janeiro, and Seoul. Regenerate an intentionally newer static snapshot
with:

```bash
npm run data:earth-landmarks
```

The generated metadata records the retrieval timestamp and source licenses: Wikidata data under
[CC0](https://www.wikidata.org/wiki/Wikidata:Licensing), Wikipedia content under
[CC BY-SA 4.0](https://en.wikipedia.org/wiki/Wikipedia:Copyrights), and GeoNames fallbacks under
...
ascension 6 h, and +Z toward the north celestial pole. The v3 binary also retains HYG `vx`, `vy`,
and `vz` in parsecs per Julian year. HYG derives these components from proper motion and radial
velocity when available.

At runtime, the engine applies the standard
[uniform rectilinear motion](https://gea.esac.esa.int/archive/documentation/GEDR3/Data_processing/chap_cu3ast/sec_cu3ast_intro/ssec_cu3ast_intro_tansforms.html)
model relative to the Solar-system barycenter: `b(T) = b(T0) + (T - T0) v`. This extrapolation is
bounded to ±10,000 Julian years around J2000; dates outside the model domain use the nearest boundary
and report that clamp. The source position and velocity are `observed`, while the propagated position
is explicitly `extrapolated`. The model does not add parallax, Galactic acceleration, or unresolved
binary motion.
...
identifier. At load time, the engine resolves that link to the same observed J2000 vector and
velocity used by the batched point, observer sky, constellation lines, search index, label, picking,
focus, and shareable URL. A featured star therefore does not create a duplicate Three.js object or a
