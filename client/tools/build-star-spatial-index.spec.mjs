...
test('builds deterministic Gaia root aggregates and measured child samples', () => {
  ...
  assert.equal(hierarchy.index.version, '5.0.0');
...
test('keeps the brightest Gaia sources and a stable uniform leaf sample independent of input order', () => {
  ...
  assert.deepEqual(forward.sourceIds, reverse.sourceIds);
  assert.deepEqual(forward.sourceIds.slice(0, 2), ['1', '2']);
...
test('keeps every committed Gaia aggregate and measured sample pack consistent with its index', async () => {
  ...
  for (const [url, nodes] of nodesByUrl) {
    ...
    for (const tile of pack.tiles) {
      ...
      if (tile.representation === 'sampled-source') {
        assert.equal(tile.sourceIds.length, tile.clusterCount);
        assert.equal(new Set(tile.sourceIds).size, tile.clusterCount);
      } else {
        assert.equal(tile.sourceIds, undefined);
      }
      assert.equal(
