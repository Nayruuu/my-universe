...
test('parseRenderingPrewarmStats keeps preparatory wall-clock diagnostics separate from frames', () => {
  assert.deepEqual(
    parseRenderingPrewarmStats({
      initialScene: { status: 'ready', durationMs: '37.5' },
  ...
  assert.throws(() => parseRenderingPrewarmStats(null), /operation stats/u);
  assert.throws(
    () =>
      parseRenderingPrewarmStats({
        initialScene: { status: 'unknown', durationMs: 1 },
        tempelScene: { status: 'idle', durationMs: null },
      }),
    /initial scene.*valid status/u,
  );
  assert.throws(
    () =>
      parseRenderingPrewarmStats({
        initialScene: { status: 'ready', durationMs: -1 },
        tempelScene: { status: 'idle', durationMs: null },
      }),
    /initial scene duration/u,
  );
});
