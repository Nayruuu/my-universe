...
export function parseRenderingPrewarmStats(value) {
  if (!value || typeof value !== 'object') {
    throw new Error('Rendering prewarm diagnostics require operation stats.');
  }

  return {
    initialScene: parseRenderingPrewarmOperation(value.initialScene, 'initial scene'),
    tempelScene: parseRenderingPrewarmOperation(value.tempelScene, 'Tempel scene'),
  };
}
...
async function measureJourney(context, baseUrl, quality, warmUp, cpuThrottleRate, labelsEnabled) {
  ...
  const rawRenderingPrewarm = await page.evaluate(
    () => window.__UNIVERSE_MAP_OBSERVABILITY__?.getRenderingPrewarmStats() ?? null,
  );
  const durations = samples.map((sample) => sample.durationMs);
  const adaptiveRendering = parseAdaptiveRenderingStats(rawAdaptiveRendering);
  const renderingPrewarm = parseRenderingPrewarmStats(rawRenderingPrewarm);
...
function parseRenderingPrewarmOperation(value, label) {
  const statuses = ['idle', 'running', 'ready', 'failed'];

  if (!value || typeof value !== 'object' || !statuses.includes(value.status)) {
    throw new Error(`Rendering prewarm ${label} requires a valid status.`);
  }

  return {
    status: value.status,
    durationMs: nullableNonNegativeNumber(value.durationMs, `Rendering prewarm ${label} duration`),
  };
}

function formatPrewarmOperation(operation) {
  return `${operation.status} · ${
    operation.durationMs === null ? '—' : `${operation.durationMs.toFixed(2)} ms`
  }`;
}
...
function printSummaries(summaries) {
  console.table(
    summaries.map((summary) => ({
      ...
      initialWarmup: formatPrewarmOperation(summary.renderingPrewarm.initialScene),
      tempelWarmup: formatPrewarmOperation(summary.renderingPrewarm.tempelScene),
