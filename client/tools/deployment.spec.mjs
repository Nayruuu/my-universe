...
test('the Azure deployment revalidates mutable astronomical data and textures', async () => {
  const configuration = JSON.parse(
    await readRepositoryFile('client/public/staticwebapp.config.json'),
  );

  for (const route of ['/data/*', '/textures/*']) {
    assert.deepEqual(configuration.routes.find((entry) => entry.route === route)?.headers, {
      'Cache-Control': 'no-cache',
    });
  }
});
