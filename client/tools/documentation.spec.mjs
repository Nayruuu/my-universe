...
test('configures the guide locales, SEO subpath, and local search', async () => {
  ...
  assert.match(config, /path\(['"]roadmap['"]\)/u);
  for (const locale of LOCALES) {
...
test('archives the clean performance evidence without conflating the later security revision', async () => {
  const root = new URL('../../docs/performance/2026-09-11-796fe70/', import.meta.url);
  const manifest = JSON.parse(await readFile(new URL('campaign.json', root), 'utf8'));
  const source = { commit: '796fe70310b28ee17c1a0cd9445688d65f05b36c', dirty: false };
  ...
  for (const profile of manifest.profiles) {
    assert.equal(profile.protocols.length, 5);
    for (const protocol of profile.protocols) {
      const bytes = await readFile(new URL(protocol.file, root));
      const report = JSON.parse(bytes.toString('utf8'));

      assert.equal(createHash('sha256').update(bytes).digest('hex'), protocol.sha256);
      assert.deepEqual(report.source, source);
      assert.equal(protocol.withinBudget, true);
      assert.deepEqual(report.summary, protocol.summary);
    }
  }
  for (const locale of LOCALES) {
    const roadmap = await readGuidePage(`${locale.prefix}roadmap/`);

    assert.match(roadmap, /`796fe70`/u);
    assert.match(roadmap, /`c380bce`/u);
    assert.match(roadmap, /\*\*10\/10\*\*/u);
  }
});
