...
import {
  locateEarthObserver,
  type EarthObserverGeolocationIssue,
} from './earth-observer-geolocation';
import {
...
type GeolocationState = 'idle' | 'locating' | 'located' | EarthObserverGeolocationIssue;
...
export class EarthObserverLocationPickerComponent {
  ...
  public readonly geolocationEnabled = input(false);
  public readonly locationChange = output<string>();
  public readonly currentPositionChange = output<EarthObserverLocation>();

  protected readonly i18n = inject(I18nService);
  protected readonly query = signal('');
  protected readonly geolocationState = signal<GeolocationState>('idle');
  protected readonly locating = computed(() => this.geolocationState() === 'locating');
  protected readonly geolocationMessage = computed(() => {
    const text = this.i18n.content().stellarObservation;
    const messages: Readonly<Record<GeolocationState, string | null>> = {
      idle: null,
      locating: text.geolocationLocating,
      located: text.geolocationLocated,
      unsupported: text.geolocationUnsupported,
      'permission-denied': text.geolocationPermissionDenied,
      'position-unavailable': text.geolocationUnavailable,
      timeout: text.geolocationTimeout,
      'invalid-position': text.geolocationInvalid,
    };

    return messages[this.geolocationState()];
  });
  ...
  protected useCurrentPosition(menu: HTMLDetailsElement): void {
    if (this.locating()) {
      return;
    }
    this.geolocationState.set('locating');
    const text = this.i18n.content().stellarObservation;
    const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;

    void locateEarthObserver(navigator.geolocation, text.myLocation, timeZone).then((result) => {
      if (result.location) {
        this.currentPositionChange.emit(result.location);
        this.geolocationState.set('located');
        this.query.set('');
        menu.open = false;
      } else {
        this.geolocationState.set(result.issue);
      }
    });
  }
}
