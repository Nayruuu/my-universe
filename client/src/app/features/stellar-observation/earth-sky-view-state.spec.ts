...
describe('EarthSkyViewState', () => {
  const navigation = { setViewMode: vi.fn() };
  ...
  it('n’ouvre la vue terrestre que pour une cible demandée explicitement', () => {
    ...
    state.open('sirius', 'Sirius', null, 18, 96);
    expect(navigation.setViewMode).toHaveBeenLastCalledWith('planetarium');
    ...
    expect(state.entryVerticalFieldOfViewDegrees()).toBe(96);
  ...
  it('conserve la cible astronomique pendant toute la vue terrestre', () => {
    ...
    state.beginJourney(target.id, target.name, target, 72.1, 94);
    expect(navigation.setViewMode).toHaveBeenLastCalledWith('planetarium');
