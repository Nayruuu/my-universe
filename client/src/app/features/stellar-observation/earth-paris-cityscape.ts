import { createEarthUrbanLightPools, type EarthUrbanLightPool } from './earth-city-lighting';
...
const PARIS_DECORATIVE_ROOFTOP_MAX_HEIGHT_UNITS = 12;

export const PARIS_DECORATIVE_SKYLINE_MAX_HEIGHT_UNITS =
  70 + PARIS_DECORATIVE_ROOFTOP_MAX_HEIGHT_UNITS;
...
export const PARIS_LIGHT_POOLS: readonly EarthUrbanLightPool[] = createEarthUrbanLightPools(
  0x7061_7269,
  'dense',
);
...
function parisRoofPath(
  ...
  switch (roof) {
    ...
    default:
      return `L${format(x)} ${format(y + 8)}H${format(midpoint - 1.5)}L${format(midpoint)} ${format(y - PARIS_DECORATIVE_ROOFTOP_MAX_HEIGHT_UNITS)}L${format(midpoint + 1.5)} ${format(y + 8)}H${format(end)}`;
  }
}
...
function flatRoofPath(
  ...
  const chimneyHeight = 4 + random() * (PARIS_DECORATIVE_ROOFTOP_MAX_HEIGHT_UNITS - 4);
