...
import { resolveObjectReceivedLight } from '../../../engine/simulation/received-light-time';
...
export interface ObjectDetailsPresentationContext {
  ...
  readonly formatDate: (time: UniverseTime) => string;
...
function createBasePresentation(context: ObjectDetailsPresentationContext) {
  return {
    ...
    receivedLight(object: SpaceObject): ReceivedLightPresentation | null {
      if (context.temporalMode() !== 'observable') {
        return null;
      }
      const received = resolveObjectReceivedLight(object, context.currentTime());
...
export interface ReceivedLightPresentation {
...
function formatLightTravelTime(
  context: ObjectDetailsPresentationContext,
  lightTravelDays: number,
): string {
  const details = context.content().details;
  const seconds = lightTravelDays * 86_400;

  if (seconds < 120) {
    return context.interpolate(details.lightSeconds, {
      value: context.formatNumber(seconds, 2),
    });
  }
  const minutes = seconds / 60;

  if (minutes < 180) {
    return context.interpolate(details.lightMinutes, {
      value: context.formatNumber(minutes, 2),
    });
  }
  if (lightTravelDays < 730) {
    return context.interpolate(details.lightDays, {
      value: context.formatNumber(lightTravelDays, 2),
    });
  }

  return context.interpolate(details.lightYears, {
    value: context.formatNumber(lightTravelDays / 365.25, 2),
  });
}
