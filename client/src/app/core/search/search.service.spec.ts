...
describe('SearchService', () => {
  ...
  it('publie atomiquement un index volumineux après avoir rendu la main au navigateur', async () => {
    const service = TestBed.inject(SearchService);
    const entries = Array.from({ length: 2_100 }, (_, entryIndex) => ({
      id: `catalog-${entryIndex}`,
      name: `Catalogue profond ${entryIndex}`,
      aliases: [],
      type: 'star' as const,
    }));
    ...
    expect(service.search('catalogue profond 2099')).toEqual([]);

    await vi.waitFor(() => expect(service.revision()).toBe(1));
    expect(service.search('catalogue profond 2099')[0]?.id).toBe('catalog-2099');
  });

  it('annule un index volumineux périmé lorsqu’un nouveau catalogue arrive', async () => {
    const service = TestBed.inject(SearchService);
    const staleEntries = Array.from({ length: 2_100 }, (_, entryIndex) => ({
      id: `stale-${entryIndex}`,
      name: `Ancien catalogue ${entryIndex}`,
      aliases: [],
      type: 'star' as const,
    }));

    service.setData([], staleEntries);
    service.setData([object('earth', 'Terre')]);
