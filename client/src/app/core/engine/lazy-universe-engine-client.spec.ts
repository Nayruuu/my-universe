...
describe('LazyUniverseEngineClient', () => {
  ...
  it('ne charge Three.js qu’à l’initialisation puis délègue toute l’API de façade', async () => {
    ...
    engine.setEarthObserverCelestialPresentations(celestialPresentations);
    ...
    expect(engine.renderingPrewarmStats).toEqual({
    ...
    expect(engine.getNavigationDebugTrace()).toEqual([]);
    engine.setNavigationDebugTracing(true);
    engine.clearNavigationDebugTrace();
    ...
    expect(() => engine.resolveObject('earth')).toThrow('pas encore initialisé');
    ...
    expect(fake.engine.setEarthObserverCelestialPresentations).toHaveBeenCalledWith(
      celestialPresentations,
    );
    expect(fake.engine.setNavigationDebugTracing).toHaveBeenCalledWith(true);
    expect(fake.engine.initialize).toHaveBeenCalledWith(container, options);
    engine.setNavigationDebugTracing(false);
    expect(fake.engine.setNavigationDebugTracing).toHaveBeenLastCalledWith(false);
    ...
    engine.setEarthObserverCelestialPresentations([]);
    engine.zoomBy(0.72);
    engine.clearNavigationDebugTrace();
    ...
    expect(fake.engine.setEarthObserverCelestialPresentations).toHaveBeenLastCalledWith([]);
    ...
    expect(engine.renderingPrewarmStats).toBe(fake.engine.renderingPrewarmStats);
    ...
    expect(engine.getNavigationDebugTrace()).toEqual(fake.navigationDebugTrace);
    expect(fake.engine.clearNavigationDebugTrace).toHaveBeenCalledOnce();
...
function createFakeEngine(): FakeEngineHarness {
  ...
  const navigationDebugTrace = [
    { sequence: 1, timestamp: '2026-08-19T18:00:00.000Z' },
  ] as unknown as ReturnType<UniverseEngineClient['getNavigationDebugTrace']>;
  ...
  const engine: UniverseEngineClient = {
    ...
    resolveObject: vi.fn(async (objectId: string) =>
      objectId === 'earth'
        ? ({ id: 'earth', name: 'Terre' } as Awaited<
            ReturnType<UniverseEngineClient['resolveObject']>
          >)
        : null,
    ),
    ...
    setNavigationDebugTracing: vi.fn(),
    getNavigationDebugTrace: vi.fn(() => navigationDebugTrace),
    clearNavigationDebugTrace: vi.fn(),
...
interface FakeEngineHarness {
  ...
  navigationDebugTrace: ReturnType<UniverseEngineClient['getNavigationDebugTrace']>;
