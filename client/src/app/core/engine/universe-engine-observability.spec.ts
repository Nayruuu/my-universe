...
describe('installUniverseEngineObservability', () => {
  ...
  it('publie un contrat typé, le délègue puis le nettoie sans écraser un successeur', () => {
    ...
    expect(bridge.getRenderingPrewarmStats()).toEqual(renderingPrewarmStats());
    ...
    expect(bridge.getNavigationDebugTrace()).toEqual([]);
    bridge.clearNavigationDebugTrace();
...
function engine() {
  return {
    ...
    renderingPrewarmStats: renderingPrewarmStats(),
...
function renderingPrewarmStats() {
  return {
    initialScene: { status: 'ready' as const, durationMs: 18 },
