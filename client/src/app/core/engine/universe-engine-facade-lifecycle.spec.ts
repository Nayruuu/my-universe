...
describe('UniverseEngineFacadeLifecycle', () => {
  it('mutualise l’initialisation et applique les valeurs par défaut', async () => {
    ...
    expect(harness.engine.ensureObjectAvailable).toHaveBeenCalledWith('earth');
  ...
  it('attend un catalogue différé avant de restaurer une cible partagée', async () => {
    const harness = createHarness({
      navigation: { targetId: 'nasa-planet-kepler-22-b', zoom: 42 },
    });

    harness.engine.hasObject = vi.fn(
      (objectId: string) => objectId === 'earth' || objectId === 'nasa-planet-kepler-22-b',
    );
    await harness.lifecycle.initialize(document.createElement('div'));

    expect(harness.engine.ensureObjectAvailable).toHaveBeenCalledWith('nasa-planet-kepler-22-b');
  ...
  it('restaure une sélection distincte sans remplacer une sélection identique', async () => {
    ...
    expect(distinct.engine.ensureObjectAvailable).toHaveBeenNthCalledWith(1, 'mars');
    expect(distinct.engine.ensureObjectAvailable).toHaveBeenNthCalledWith(2, 'venus');
    ...
    expect(identical.engine.ensureObjectAvailable).toHaveBeenCalledOnce();
  ...
  it('ignore aussi une sélection différée résolue après la destruction', async () => {
    const harness = createHarness({
      navigation: { targetId: 'earth', selectedId: 'kepler-452-b' },
    });
    const selectionGate = deferred<boolean>();

    harness.engine.ensureObjectAvailable
      .mockResolvedValueOnce(true)
      .mockReturnValueOnce(selectionGate.promise);
    const staleInitialization = harness.lifecycle.initialize(document.createElement('div'));

    await vi.waitFor(() => expect(harness.engine.ensureObjectAvailable).toHaveBeenCalledTimes(2));
    harness.lifecycle.dispose();
    selectionGate.resolve(true);
    await staleInitialization;

    expect(harness.engine.completeTargetTransition).not.toHaveBeenCalled();
    expect(harness.engine.selectObject).not.toHaveBeenCalled();
    expect(harness.engine.start).not.toHaveBeenCalled();
    expect(harness.setError).not.toHaveBeenCalled();
  });
});
