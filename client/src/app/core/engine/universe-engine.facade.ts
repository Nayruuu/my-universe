...
import { MILKY_WAY_REVIEW_STOPS, type MilkyWayReviewStop } from './milky-way-review-stops';
import {
...
export type MilkyWayReviewPlaybackResult = 'completed' | 'interrupted' | 'failed';

const MILKY_WAY_REVIEW_POLL_INTERVAL_MS = 16;
const MILKY_WAY_REVIEW_TRANSITION_TIMEOUT_MS = 10_000;
const MILKY_WAY_REVIEW_DISTANCE_TOLERANCE = 0.02;
...
export class UniverseEngineFacade {
  ...
  public readonly milkyWayReviewPlaying = signal(false);
  public readonly milkyWayReviewStop = signal<MilkyWayReviewStop | null>(null);
  ...
  /**
   * Replays fixed scale checkpoints through the ordinary target-transition controller. It is a
   * debug aid only: no camera duration, easing, or navigation policy is overridden here.
   */
  public async playMilkyWayReview(): Promise<MilkyWayReviewPlaybackResult> {
    if (this.milkyWayReviewPlaying()) {
      return 'interrupted';
    }
    ...
    this.milkyWayReviewPlaying.set(true);

    try {
      for (const stop of MILKY_WAY_REVIEW_STOPS) {
        this.milkyWayReviewStop.set(stop);
        await this.engine.setTarget(stop.targetId, stop.zoom, stop.orientation);

        const arrived = await this.waitForMilkyWayReviewStop(stop);
      ...
      this.error.set(this.i18n.content().facade.targetUnavailable);

      return 'failed';
    } finally {
      this.milkyWayReviewStop.set(null);
      this.milkyWayReviewPlaying.set(false);
    }
  }

  public async copyMilkyWayReviewUrl(): Promise<'copied' | 'failed'> {
  ...
  private waitForMilkyWayReviewStop(stop: MilkyWayReviewStop): Promise<boolean> {
    return new Promise((resolve) => {
      let elapsed = 0;
      const poll = () => {
        if (!this.engine.cameraTransitioning) {
          resolve(this.isMilkyWayReviewStopSettled(stop));

          return;
        }
        if (elapsed >= MILKY_WAY_REVIEW_TRANSITION_TIMEOUT_MS) {
          resolve(false);

          return;
        }
        elapsed += MILKY_WAY_REVIEW_POLL_INTERVAL_MS;
        window.setTimeout(poll, MILKY_WAY_REVIEW_POLL_INTERVAL_MS);
      };
  ...
  private isMilkyWayReviewStopSettled(stop: MilkyWayReviewStop): boolean {
    return (
      Math.abs(this.engine.cameraDistance - stop.zoom) <=
      Math.max(Number.EPSILON, stop.zoom * MILKY_WAY_REVIEW_DISTANCE_TOLERANCE)
    );
  }
