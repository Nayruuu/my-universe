...
export class UniverseEclipsePresenter {
  ...
  public formatContactSummary(event: EarthEclipseEvent | null): string | null {
    if (event?.scope !== 'local' || !event.localContacts) {
      return null;
    }
    const facade = this.context.getContent().facade;
    const contacts = event.localContacts;
    const values = [
      this.formatContact(facade.contactFirst, contacts.partialBegin, event),
      contacts.centralBegin
        ? this.formatContact(facade.contactCentralBegin, contacts.centralBegin, event)
        : null,
      this.formatContact(facade.contactMaximum, contacts.maximum, event),
      contacts.centralEnd
        ? this.formatContact(facade.contactCentralEnd, contacts.centralEnd, event)
        : null,
      this.formatContact(facade.contactLast, contacts.partialEnd, event),
    ];
  ...
  private formatContact(
    label: string,
    contact: NonNullable<EarthEclipseEvent['localContacts']>['maximum'],
    ...
    const horizon = contact.aboveHorizon
      ? ''
      : ` (${this.context.getContent().facade.belowHorizon})`;

    return `${label} ${clock}${horizon}`;
  }
}
