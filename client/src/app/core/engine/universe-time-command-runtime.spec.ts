...
describe('UniverseTimeCommandRuntime', () => {
  ...
  it('applique une vitesse et parcourt les vitesses disponibles dans leurs limites', () => {
    ...
    const dayIndex = TIME_SPEED_OPTIONS.findIndex((option) => option.id === 'day');

    harness.state.speed = TIME_SPEED_OPTIONS[dayIndex]!.daysPerSecond;
    harness.runtime.cycleSpeed(1);
    expect(harness.state.speed).toBe(TIME_SPEED_OPTIONS[dayIndex + 1]!.daysPerSecond);
  });
