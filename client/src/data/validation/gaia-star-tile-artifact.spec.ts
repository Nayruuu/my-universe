...
describe('artefacts stellaires Gaia DR3', () => {
  ...
  it('valide les échantillons mesurés du paquet raffiné autour du Soleil', () => {
    ...
    for (const tile of pack.tiles) {
      ...
      expect(tile.sourceIds).toHaveLength(tile.clusterCount);
      expect(tile.sourceIds?.every((sourceId) => /^\d+$/u.test(sourceId))).toBe(true);
      expect(new Set(tile.sourceIds).size).toBe(tile.clusterCount);
