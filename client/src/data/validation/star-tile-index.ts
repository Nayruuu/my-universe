...
export function parseStarClusterTile(value: unknown, source: string): StarClusterTile {
  ...
  if (
    ...
    (value['representation'] === 'sampled-source'
      ? !isGaiaSourceIdArray(sourceIds) ||
        sourceIds.length !== clusterCount ||
        new Set(sourceIds).size !== sourceIds.length
      : sourceIds !== undefined)
  ) {
    throw invalidTile(source);
  }
  const parsedSourceIds = isGaiaSourceIdArray(sourceIds) ? sourceIds : undefined;
  ...
  return {
    ...
    ...(parsedSourceIds === undefined ? {} : { sourceIds: parsedSourceIds }),
  };
}
...
function isGaiaSourceIdArray(value: unknown): value is string[] {
  return isStringArray(value) && value.every((sourceId) => /^\d+$/u.test(sourceId));
}
