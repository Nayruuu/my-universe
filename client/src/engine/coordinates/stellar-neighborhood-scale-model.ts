...
// Presentation thresholds calibrated against the unchanged Galactic approach: the near cloud
// becomes dense around 600 scene units. Let it surround the observer before map annotations.
// The catalogue still unfolds invisibly at 3,600–2,400 and reveals over a full decade of zoom.
export const STELLAR_NEIGHBORHOOD_REVEAL_START = 90;
export const STELLAR_NEIGHBORHOOD_REVEAL_END = 900;
export const PLANETARY_SYSTEM_REVEAL_OUTER_DISTANCE = 240;
export const PLANETARY_SYSTEM_REVEAL_INNER_DISTANCE = 90;
...
const PLANETARY_SYSTEM_REVEAL_BEGIN = calculateStellarNeighborhoodReveal(
  PLANETARY_SYSTEM_REVEAL_OUTER_DISTANCE,
);
const PLANETARY_SYSTEM_REVEAL_COMPLETE = calculateStellarNeighborhoodReveal(
...
/**
 * Reserves an interstellar stage before ordinary planets, orbits, and their labels appear.
 * This consumes the existing presentation progress, not camera motion or catalogue coordinates.
 * Active objects remain readable through the callers' existing selection/focus exceptions.
 */
export function calculatePlanetarySystemReveal(stellarNeighborhoodReveal: number): number {
  return smoothstep(
    PLANETARY_SYSTEM_REVEAL_BEGIN,
    PLANETARY_SYSTEM_REVEAL_COMPLETE,
    Number.isNaN(stellarNeighborhoodReveal) ? 0 : stellarNeighborhoodReveal,
  );
}

/** Keeps the active star readable at its existing focus distance during the longer field reveal. */
export function calculateActiveStellarDetailReveal(cameraDistance: number): number {
  return calculateStellarReveal(cameraDistance, 520, 2_200);
}
...
function calculateStellarReveal(
  cameraDistance: number,
  ...
  const collapseProgress = smoothstep(
    Math.log(innerDistance),
    Math.log(outerDistance),
    Math.log(Math.max(innerDistance, normalizedDistance)),
  );
