...
describe('éclipses terrestres', () => {
  ...
  it('déplie les empreintes partielles dans les deux sens autour de l’antiméridien', () => {
    const time = { julianDay: JULIAN_DAY_J2000 };
    const renderData = createSolarEclipseEventMapRenderData({
      ...
      partialFootprints: [
        {
          time,
          center: eclipseMapPoint(180),
          boundary: [eclipseMapPoint(170), eclipseMapPoint(-170)],
        },
        {
          time,
          center: eclipseMapPoint(-180),
    ...
    expect(renderData.partialPolygons[0]![1]!.x).toBeGreaterThan(1);
    expect(renderData.partialPolygons[1]![1]!.x).toBeLessThan(0);
  });
...
function eclipseMapPoint(longitude: number) {
  return {
    latitude: 0,
    longitude,
