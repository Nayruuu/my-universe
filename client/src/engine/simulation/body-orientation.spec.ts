...
describe('orientation axiale IAU', () => {
  it.each([
    ...
    ['phobos', { x: 0.43747068, y: 0.8983582, z: 0.03964787 }],
    ['deimos', { x: 0.46007961, y: 0.88640966, z: 0.05103585 }],
  ...
  it('fait tourner Mars au rythme sidéral documenté, indépendamment du rendu', () => {
    const afterOneHour = calculateBodyOrientation({ julianDay: J2000.julianDay + 1 / 24 }, 'mars');
    const afterOneSiderealRotation = calculateBodyOrientation(
      { julianDay: J2000.julianDay + 24.622962 / 24 },
      'mars',
    );
    const initial = calculateBodyOrientation(J2000, 'mars');

    expect(angleDegrees(initial.xAxis, afterOneHour.xAxis)).toBeCloseTo(14.6205, 3);
    expect(angleDegrees(initial.xAxis, afterOneSiderealRotation.xAxis)).toBeLessThan(0.01);
  });
  ...
  it('borne les rotations hors du domaine documenté des éphémérides', () => {
    const atBoundary = calculateBodyOrientation(
      { julianDay: ASTRONOMY_ENGINE_MAX_JULIAN_DAY },
      'earth',
    );
    const oneMillionYearsLater = calculateBodyOrientation(
      { julianDay: ASTRONOMY_ENGINE_MAX_JULIAN_DAY + 365_250_000 },
      'earth',
    );

    expectVector(oneMillionYearsLater.xAxis, atBoundary.xAxis, 10);
    expectVector(oneMillionYearsLater.yAxis, atBoundary.yAxis, 10);
    expectVector(oneMillionYearsLater.zAxis, atBoundary.zAxis, 10);
  });

  it('résout tous les corps disposant d’un modèle de rotation scientifique', () => {
    const supported = [
      'sun',
      'mercury',
      'venus',
...
function angleDegrees(first: Vector3Like, second: Vector3Like): number {
