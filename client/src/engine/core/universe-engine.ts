...
import type {
  ...
  DisplayOptions,
  GraphicQuality,
  RenderingPrewarmPerformanceStats,
  SpaceObject,
  UniverseEngineEvent,
  UniverseTime,
  Vector3Like,
...
import { RenderingPrewarmPerformanceTrace } from '../performance/rendering-prewarm-performance-trace';
...
export class UniverseEngine {
  ...
  private readonly renderingPrewarmPerformance = new RenderingPrewarmPerformanceTrace();
  ...
  private readonly tempelFilamentLoader = new UniverseTempelFilamentLoader({
    ...
    prewarmScene: async () => {
      ...
      this.renderingPrewarmPerformance.begin('tempelScene');
      try {
        const ready = await scene.prewarmSceneMaterials(renderer, camera);

        this.renderingPrewarmPerformance.complete('tempelScene', ready);
      } catch (error) {
        this.renderingPrewarmPerformance.complete('tempelScene', false);
        throw error;
      }
    },
  ...
  private readonly debugRuntime = new UniverseDebugRuntime(this.objectRuntime, {
    ...
    getRenderingPrewarm: () => this.renderingPrewarmPerformance.snapshot,
  ...
  constructor(
    ...
    this.initializationBootstrap = new UniverseInitializationBootstrap(
      ...
      this.renderingPrewarmPerformance,
    );
  }
  ...
  public get renderingPrewarmStats(): RenderingPrewarmPerformanceStats {
    return this.renderingPrewarmPerformance.snapshot;
  }
  ...
  private releaseRuntimeResources(): void {
    ...
    this.renderingPrewarmPerformance.reset();
