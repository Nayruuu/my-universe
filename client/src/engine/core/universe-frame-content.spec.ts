...
describe('UniverseFrameContent', () => {
  ...
  it('précharge les filaments dès le voisinage stellaire sans les installer dans la scène', () => {
    const harness = createHarness({ streamingAvailable: false });

    harness.content.update(0.1, harness.services, 4);

    expect(harness.updateStreaming).not.toHaveBeenCalled();
    expect(harness.ensureMilkyWayAtlas).toHaveBeenCalledOnce();
    expect(harness.preloadTempelFilamentSpines).toHaveBeenCalledOnce();
    expect(harness.ensureTempelFilamentSpines).not.toHaveBeenCalled();
    expect(harness.updateObjectLod).toHaveBeenCalledWith(harness.camera, 720, 4, 0.1, true);
    expect(harness.updateSceneLod).toHaveBeenCalledWith(
      4,
      0.1,
      24,
  ...
  it('continue de précharger les filaments à l’approche galactique et dans l’Univers proche', () => {
    const harness = createHarness({ streamingAvailable: false });

    harness.content.update(0.1, harness.services, 3);
    harness.content.update(0.1, harness.services, 4);
    harness.content.update(0.1, harness.services, 5);

    expect(harness.preloadTempelFilamentSpines).toHaveBeenCalledTimes(3);
    expect(harness.ensureTempelFilamentSpines).not.toHaveBeenCalled();
  });
