...
describe('UniverseFrameContent', () => {
  it('synchronise le streaming, précharge la Voie lactée et actualise les LOD proches', () => {
    ...
    expect(harness.updateObjectLod).toHaveBeenCalledWith(harness.camera, 720, 2, 0.25);
    expect(harness.updateSceneLod).toHaveBeenCalledWith(2, 0.25, 24, harness.camera.position);
  });

  it('précharge les filaments au niveau cosmique sans demander l’atlas galactique', () => {
    ...
    expect(harness.updateObjectLod).toHaveBeenCalledWith(harness.camera, 720, 6, 0.1);
    expect(harness.updateSceneLod).toHaveBeenCalledWith(6, 0.1, 24, harness.camera.position);
  });
