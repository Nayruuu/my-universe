  ...
  public async create(
    ...
    try {
      ...
      if (supportsRenderingPrewarm(rendering.renderer)) {
        this.renderingPrewarmPerformance.begin('initialScene');
        try {
          const ready = await sceneRuntime.scene.prewarmInitialRendering(
            rendering.renderer,
            rendering.camera,
          );

          this.renderingPrewarmPerformance.complete('initialScene', ready);
        } catch (error) {
          this.renderingPrewarmPerformance.complete('initialScene', false);
          throw error;
        }
