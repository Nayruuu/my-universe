...
export interface UniverseTempelFilamentScene {
  prepareTempelFilamentSpineCatalog(
...
export class UniverseTempelFilamentLoader {
  ...
  public preload(): Promise<void> {
    ...
    this.preloadPromise = Promise.all([this.catalogPromise, this.rendererPromise])
      .then(async ([catalog, renderer]) => {
        if (!catalog || !renderer.loaded || !this.bindings.isContextCurrent(context)) {
          return;
        }
        await context.scene.prepareTempelFilamentSpineCatalog(
          catalog,
          context.registry,
        ...
        await this.bindings.prewarmScene();
      })
      .catch((error: unknown) => {
