...
describe('UniverseFrameSimulation', () => {
  it('laisse une frame en pause intacte', () => {
    ...
    expect(harness.updateStellarCatalog).not.toHaveBeenCalled();
  ...
  it('cadence les positions et les événements temporels indépendamment du rendu', () => {
    ...
    expect(harness.updateStellarCatalog).toHaveBeenCalledWith(
      harness.timeController.currentTime,
      'observable',
    );
  ...
  it('accepte un registre secondaire absent', () => {
    ...
    expect(harness.updateStellarCatalog).toHaveBeenCalledWith(resetTime, 'observable');
    expect(harness.followCurrentTarget).toHaveBeenCalledOnce();
  ...
  it('applique immédiatement une date à tous les registres et publie le nouvel état', () => {
    ...
    expect(harness.updateStellarCatalog).toHaveBeenCalledWith(requestedTime, 'observable');
  ...
  it('rafraîchit immédiatement la géométrie lors d’un changement de mode temporel', () => {
    const harness = createHarness();

    harness.simulation.refreshTemporalPresentation(harness.registry);
    ...
    harness.simulation.refreshTemporalPresentation(null);
    expect(harness.followCurrentTarget).toHaveBeenCalledTimes(2);
  });
});
...
function createHarness(withExoplanetRegistry = true): SimulationHarness {
  ...
  const updateStellarCatalog = vi.fn();
  ...
  const simulation = new UniverseFrameSimulation(timeController, {
    ...
    getTemporalMode: () => 'observable',
    updateStellarCatalog,
