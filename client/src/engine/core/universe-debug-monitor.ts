import {
  ...
  type RenderingPrewarmPerformanceStats,
...
export interface UniverseDebugContext {
  ...
  readonly quality: EngineDebugStats['quality'];
  readonly pixelRatio: number;
  readonly adaptiveRendering: AdaptiveRenderingStats;
  ...
  readonly renderingPrewarm: RenderingPrewarmPerformanceStats;
...
export class UniverseDebugMonitor {
  ...
  public update(deltaSeconds: number): void {
    ...
    this.bindings.emitStats(this.createStats(fps, context));
    this.statsAccumulator = 0;
    this.statsFrames = 0;
  }

  private createStats(fps: number, context: UniverseDebugContext): EngineDebugStats {
    ...
    return {
      ...
      adaptiveRendering: { ...context.adaptiveRendering },
      zoom:
        context.zoomDiagnostics && context.zoomAnchor
          ? { ...context.zoomDiagnostics, ...context.zoomAnchor }
          : null,
      startupPerformance: {
        ...context.startupPerformance,
        exceededBudgets: [...context.startupPerformance.exceededBudgets],
      },
      tempelPerformance: { ...context.tempelPerformance },
      renderingPrewarm: {
        initialScene: { ...context.renderingPrewarm.initialScene },
        tempelScene: { ...context.renderingPrewarm.tempelScene },
      },
    };
  }
}
