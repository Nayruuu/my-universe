...
import { type CameraOrientation, type SpaceObject } from '../../data/models/universe.models';
...
export class UniverseViewController {
  ...
  public async setTarget(
    ...
    if (shouldPreloadTempelFilamentSpines(objectId)) {
      // Start the worker and shader preparation before the scale travelling begins. This is
      // intentionally fire-and-forget: navigation timing and camera motion stay identical.
    ...
    const restoredViewDirection = createRestoredViewDirection(orientation);
    const constellationDistance =
      constellationRadius !== null &&
      constellationRadius !== undefined &&
      verticalFieldOfView !== null
        ? getOrbitOverviewDistance(constellationRadius, verticalFieldOfView)
        : undefined;
    const focusDistance = zoom ?? constellationDistance ?? this.getCatalogFocusDistance(objectId);

    if (restoredViewDirection) {
      cameraController.focusOnFromDirection(
        position,
        object,
        restoredViewDirection.negate(),
        focusDistance,
      );
    } else if (constellationDistance !== undefined) {
      cameraController.focusOnFromDirection(
        position,
        object,
      ...
      cameraController.focusOnFromDirection(position, object, earthFacingDirection, focusDistance);
    } else {
      cameraController.focusOn(position, object, focusDistance);
    }
  ...
  public async viewRotation(objectId: string): Promise<void> {
    ...
    await this.setTarget(objectId, distance, undefined, 'immediate');
  }
...
function shouldPreloadTempelFilamentSpines(objectId: string): boolean {
  return objectId === 'milky-way' || objectId === 'local-group' || objectId === 'nearby-universe';
}

function createRestoredViewDirection(
  ...
  const direction = new THREE.Vector3(orientation.x, orientation.y, orientation.z);
