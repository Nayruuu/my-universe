...
describe('UniverseViewController', () => {
  ...
  it('restaure une orientation partagée comme direction de regard', async () => {
    const harness = createHarness();
    const orientation = { x: 2, y: -1, z: -2 };

    await harness.controller.setTarget('sun', 520, orientation);

    const restoredOffset = harness.cameraController.focusOnFromDirection.mock.calls.at(-1)?.[2];

    expect(restoredOffset?.distanceTo(new THREE.Vector3(-2, 1, 2).normalize())).toBeLessThan(1e-12);
    ...
    await harness.controller.setTarget('sun', 520, { x: 0, y: 0, z: 0 });
    expect(harness.cameraController.focusOn).toHaveBeenLastCalledWith(
      harness.positions.get('sun'),
      harness.definitions.get('sun'),
      520,
...
interface Harness {
  ...
  readonly preloadTempelFilamentSpines: ReturnType<typeof vi.fn>;
...
function createHarness(): Harness {
  ...
  const preloadTempelFilamentSpines = vi.fn(async () => undefined);
