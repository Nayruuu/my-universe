...
describe('UniverseFrameRuntime', () => {
  it('orchestre simulation, navigation, contenu, rendu et diagnostic dans cet ordre', () => {
    ...
    expect(harness.updateAdaptiveRendering).toHaveBeenCalledWith(0.016);
  ...
  it.each([
    ...
    expect(harness.updateAdaptiveRendering).not.toHaveBeenCalled();
...
function createHarness(): FrameRuntimeHarness {
  ...
  const updateAdaptiveRendering = vi.fn(() => phases.push('adaptive-rendering'));
...
interface FrameRuntimeHarness {
  ...
  readonly updateAdaptiveRendering: ReturnType<typeof vi.fn>;
