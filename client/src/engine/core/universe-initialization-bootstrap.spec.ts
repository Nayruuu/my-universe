...
describe('UniverseInitializationBootstrap', () => {
  ...
  it('propage un échec de préchauffage initial après l’avoir marqué comme tel', async () => {
    const harness = createHarness(true);

    harness.sceneRuntime.scene.prewarmInitialRendering.mockRejectedValueOnce(
      new Error('compilation initiale indisponible'),
    );

    await expect(harness.bootstrap.create(options(harness))).rejects.toThrow(
      'compilation initiale indisponible',
    );
