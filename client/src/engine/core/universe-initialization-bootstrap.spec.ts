...
import {
  type UniverseInitializationDependencies,
...
describe('UniverseInitializationBootstrap', () => {
  ...
  it('ne crée aucun renderer lorsque les dépendances ou données échouent', async () => {
    ...
    dependencyFailure.loadDependencies.mockRejectedValue(new Error('dependency failure'));
...
function createHarness(hasStarCatalog: boolean): InitializationHarness {
  ...
  const dependencies: UniverseInitializationDependencies = {
    loadAssets,
    createSceneRuntime,
