...
export class UniverseFrameContent {
  ...
  public update(
    ...
    // Begin the invisible Tempel preparation as the user leaves the Solar System. The catalogue,
    // geometry and shaders can then settle through the stellar and galactic journey, rather than
    // competing with the first nearby-universe frames. This deliberately changes no camera state.
    if (lodLevel >= 2 && lodLevel <= 5) {
