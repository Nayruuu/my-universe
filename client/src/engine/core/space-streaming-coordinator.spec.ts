...
describe('SpaceStreamingCoordinator', () => {
  ...
  it('ne synchronise aucun agrégat stellaire lorsque cette couche visuelle est désactivée', () => {
    const callbacks = callbackHarness();
    const stars = starTileStream();
    const coordinator = new SpaceStreamingCoordinator(null, stars.stream, callbacks.callbacks, {
    ...
    coordinator.update(streamingFrame({ lodLevel: 3 }), 1);
    coordinator.update(streamingFrame({ lodLevel: 4 }), 1);
    coordinator.invalidateViews();
    coordinator.update(streamingFrame({ lodLevel: 3 }), 1);
  ...
  it('publie la première couverture Gaia avant le raffinement déjà demandé', async () => {
    const callbacks = callbackHarness();
    ...
    const coverage = starClusterTile(4);
    const detailed = starClusterTile(3);
    const stars = starTileStream({
      synchronize: vi
        .fn<
          (view: StarTileView) => Promise<{
            changed: boolean;
            tiles: readonly StarClusterTile[];
            isCoverage?: boolean;
          }>
        >()
        .mockReturnValueOnce(first.promise)
        .mockResolvedValueOnce({ changed: true, tiles: [detailed] }),
    });
    const coordinator = new SpaceStreamingCoordinator(null, stars.stream, callbacks.callbacks);
    const frame = streamingFrame({ lodLevel: 2 });

    coordinator.update(frame, 0.25);
    coordinator.update(frame, 0.25);
    first.resolve({ changed: true, tiles: [coverage], isCoverage: true });

    await vi.waitFor(() => expect(callbacks.onStarTilesChanged).toHaveBeenCalledTimes(2));
    expect(callbacks.onStarTilesChanged).toHaveBeenNthCalledWith(1, [coverage]);
    expect(callbacks.onStarTilesChanged).toHaveBeenNthCalledWith(2, [detailed]);
  });
