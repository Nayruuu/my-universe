...
describe('UniverseFrameRenderer', () => {
  it('privilégie la cible pour la lentille et la sélection pour les labels', () => {
    ...
    expect(harness.setLabelsTransitioning).toHaveBeenCalledWith(false);
  });
  ...
  it('conserve seulement les labels du ciel dans la vue terrestre', () => {
    const harness = createHarness({
      observerModeActive: true,
      observerSkyObjectId: 'sirius',
    });

    harness.renderer.render(0.1, harness.services, 0);
    const readWorldPosition = harness.renderLabels.mock.calls[0]?.[1] as (
      objectId: string,
      target: THREE.Vector3,
    ) => THREE.Vector3 | null;

    expect(readWorldPosition('sun', new THREE.Vector3())).toBeNull();
    expect(readWorldPosition('sirius', new THREE.Vector3())).toBe(harness.worldPosition);
  });
});
...
function createHarness(overrides: Partial<HarnessOptions> = {}) {
  ...
  const setLabelsTransitioning = vi.fn();
