...
const rendererHarness = vi.hoisted(() => {
  class FakeWebGLRenderer {
    ...
    public readonly compileAsync = vi.fn(async (scene: THREE.Object3D) => scene);
...
describe('UniverseEngine', () => {
  ...
  it('branche le catalogue dense et tous les adaptateurs entre contrôles et moteur', async () => {
    ...
    const handleSemanticZoom = vi
      ...
      .mockImplementation(() => 'retain-wheel-anchor');
    ...
    const catalogVisibility = vi
      .spyOn(access.universeScene!, 'isCatalogObjectVisibleForLabels')
      .mockReturnValue(false);

    expect(labelManager.isObjectVisible('hyg-3229')).toBe(false);
    const observerPresentation = vi
      .spyOn(access.cameraController!, 'observerPresentationActive', 'get')
      .mockReturnValue(true);

    expect(labelManager.isObjectVisible('hyg-3229')).toBe(true);
    observerPresentation.mockRestore();
    catalogVisibility.mockRestore();
    ...
    expect(handleSemanticZoom).toHaveBeenCalledWith(
      'hyg-3229',
      -120,
      {
        x: 0.25,
  ...
  it('branche Cosmicflows-4 à la recherche, la sélection et la navigation cosmique', async () => {
    ...
    await expect(engine.resolveObject('cf4-pgc-42')).resolves.toMatchObject({
      id: 'cf4-pgc-42',
      name: 'Groupe PGC 42',
    });
    await expect(engine.resolveObject('missing')).resolves.toBeNull();
  ...
  it('charge les épines Tempel à la demande puis conserve une seule requête', async () => {
    ...
    expect(access.universeScene?.visibleTempelFilamentSpineSegmentCount).toBe(0);
    expect(rendererHarness.instances[0]?.compileAsync).toHaveBeenCalledOnce();
  ...
  it('prépare Tempel sans compilation GPU quand le renderer ne la propose pas', async () => {
    const runtime = createRuntime();
    ...
    runtime.catalog.cosmicStructureCatalogRegistry = { has: () => true };
    Reflect.deleteProperty(runtime.renderer, 'compileAsync');

    await (
      runtime.access as unknown as { preloadTempelFilamentSpines(): Promise<void> }
    ).preloadTempelFilamentSpines();

    expect(runtime.scene.prepareTempelFilamentSpineCatalog).toHaveBeenCalledOnce();
    expect(runtime.scene.prewarmSceneMaterials).not.toHaveBeenCalled();
    runtime.engine.dispose();
  });

  it('conserve l’échec d’un préchauffage Tempel dans les diagnostics de rendu', async () => {
    const runtime = createRuntime();
    const loader = (
      runtime.access as unknown as {
        readonly tempelFilamentLoader: {
          readonly bindings: { prewarmScene(): Promise<void> };
        };
      }
    ).tempelFilamentLoader;

    runtime.scene.prewarmSceneMaterials.mockRejectedValueOnce(
      new Error('compilation Tempel indisponible'),
    );

    await expect(loader.bindings.prewarmScene()).rejects.toThrow('compilation Tempel indisponible');
    expect(runtime.engine.renderingPrewarmStats.tempelScene.status).toBe('failed');
    runtime.engine.dispose();
  });

  it('expose la télémétrie de préchauffage du rendu sans créer de travail de rendu', () => {
    const runtime = createRuntime();

    expect(runtime.engine.renderingPrewarmStats).toEqual({
  ...
  it('précharge Tempel dès le ciblage public de la Voie lactée', async () => {
    const runtime = createRuntime();
    const preloadTempelFilamentSpines = vi
      .spyOn(runtime.access, 'preloadTempelFilamentSpines')
      .mockResolvedValue();

    installNavigationHierarchy(runtime);
    await runtime.engine.setTarget('milky-way');

    expect(preloadTempelFilamentSpines).toHaveBeenCalledOnce();
    expect(runtime.access.targetId).toBe('milky-way');
    runtime.engine.dispose();
  });
  ...
  it('expose ses valeurs temporelles, sa qualité recommandée et ses collections initiales', () => {
    ...
    expect(runtime.engine.cameraOrientation).toEqual({ x: 0.4, y: -0.2, z: -0.9 });
    runtime.engine.dispose();
  });
  ...
  it.each([false, true])(
    'anime le retour vers la Terre, mouvement réduit : %s',
    (reducedMotion) => {
      const runtime = createRuntime();
      const events: UniverseEngineEvent[] = [];
      ...
      runtime.engine.subscribe((event) => events.push(event));
      if (reducedMotion) {
        runtime.access.selectionManager = null;
      }

      runtime.engine.exitEarthObservation(true);

      expect(runtime.controller.leaveObserver).toHaveBeenCalledWith(
        new THREE.Vector3(),
        runtime.definitions.get('earth'),
        reducedMotion ? 0 : undefined,
      );
  ...
  it.each(['controller', 'observer', 'earth', 'position'])(
    'sort sans animation si le contexte nécessaire manque : %s',
    (missing) => {
      const runtime = createRuntime();

      runtime.controller.observerPresentationActive = missing !== 'observer';
      if (missing === 'controller') {
        runtime.access.cameraController = null;
      }
      if (missing === 'earth') {
        runtime.definitions.delete('earth');
      }
      if (missing === 'position') {
        runtime.registry.getWorldPosition.mockReturnValue(null);
      }

      runtime.engine.exitEarthObservation(true);

      expect(runtime.controller.leaveObserver).not.toHaveBeenCalled();
      expect(runtime.access.targetId).toBeNull();
    },
  );
  ...
  it('applique les options visuelles sans reconstruire lorsque la qualité reste identique', () => {
    ...
    runtime.engine.exitEarthObservation();
  ...
  it('transmet les présentations célestes terrestres au registre principal', () => {
    const runtime = createRuntime();
    const presentations = [
      {
        objectId: 'jupiter',
    ...
    runtime.engine.setEarthObserverCelestialPresentations(presentations);

    expect(runtime.registry.setEarthObserverCelestialPresentations).toHaveBeenLastCalledWith(
      presentations,
    );
  });
  ...
  it('publie le dernier diagnostic de zoom avec son ancre objet', () => {
    const runtime = createRuntime();
    const events: UniverseEngineEvent[] = [];
  ...
  it('conserve l’ancre pointeur quelles que soient les interceptions', () => {
    const runtime = createRuntime();

    runtime.catalog.has.mockImplementation((objectId) => objectId === 'mars');
  ...
  it('enregistre chaque intention de molette uniquement lorsque la trace est activée', () => {
    const runtime = createRuntime();
    const pointer = { x: 0.35, y: -0.2 };

    runtime.access.handleSemanticZoomIntent(null, 120, pointer);
    expect(runtime.engine.getNavigationDebugTrace()).toEqual([]);

    runtime.engine.setNavigationDebugTracing(true);
    runtime.controller.lastZoomDiagnostics = {
      deltaY: -120,
      beforeDistance: 24,
    ...
    runtime.controller.zoomSemantically.mockImplementationOnce(() => {
      runtime.camera.position.set(4, 5, 6);
      runtime.controller.distanceToTarget = 12;
    });
    runtime.access.handleSemanticZoomIntent('mars', -120, pointer);
    ...
    runtime.engine.clearNavigationDebugTrace();
    runtime.controller.zoomSemantically.mockImplementationOnce(() => {
      runtime.access.cameraController = null;
    });
    runtime.access.handleSemanticZoomIntent(null, 120, pointer);
    expect(runtime.engine.getNavigationDebugTrace()[0]?.zoom).toBeNull();
    runtime.access.cameraController = runtime.controller;

    runtime.engine.clearNavigationDebugTrace();
    runtime.access.camera = null;
    runtime.access.handleSemanticZoomIntent(null, 120, pointer);
    expect(runtime.engine.getNavigationDebugTrace()).toEqual([]);
  });
  ...
  it('traite un clic simple, un clic de focus et une désélection', async () => {
    ...
    runtime.access.handlePick(null, false);
    expect(runtime.controller.releaseTarget).toHaveBeenCalledOnce();

    runtime.access.handlePick('earth', true);
    await vi.waitFor(() => expect(runtime.access.selectedId).toBe('earth'));
    expect(runtime.registry.select).toHaveBeenLastCalledWith('earth');
    expect(runtime.controller.focusOn).toHaveBeenCalled();
  });

  it('désélectionne le fond du ciel terrestre sans quitter sa présentation', () => {
    const runtime = createRuntime();
    ...
    expect(runtime.registry.select).toHaveBeenCalledWith(null);
    expect(runtime.access.selectedId).toBeNull();
    expect(runtime.access.targetId).toBe('earth');
    expect(runtime.controller.releaseTarget).not.toHaveBeenCalled();
  });
  ...
  it('adopte l’objet au zoom avant et ne le libère au seuil que vers l’espace vide', () => {
    const runtime = createRuntime();

    runtime.access.targetId = 'earth';
    runtime.controller.semanticZoomActive = false;
    runtime.controller.inwardZoomActive = true;
    ...
    expect(runtime.controller.adoptZoomAnchor).not.toHaveBeenCalled();
    ...
    expect(runtime.controller.adoptZoomTarget).not.toHaveBeenCalled();
    expect(runtime.controller.trackTarget).toHaveBeenCalled();
    expect(runtime.controller.adoptZoomAnchor).toHaveBeenCalled();
    ...
    expect(runtime.controller.adoptZoomPointer).toHaveBeenCalledTimes(pointerCallCount + 1);
    expect(runtime.controller.adoptZoomPointer).toHaveBeenLastCalledWith(0, 0);
  ...
  it('utilise le pointeur et détache la cible même pendant un changement de référentiel', () => {
    const runtime = createRuntime();
  ...
  it('change de référentiel avec les échelles pendant un aller-retour à la molette', () => {
    ...
    runtime.controller.zoomSemantically.mockImplementation(() => {
      ...
      runtime.controller.semanticZoomActive = distances.length > 0;
    });
    ...
    expect(runtime.controller.adoptReferenceFrame).toHaveBeenCalledTimes(10);
    expect(runtime.controller.adoptReferenceFrame).toHaveBeenLastCalledWith(
  ...
  it('synchronise le contexte après un pincement géré nativement par OrbitControls', () => {
    ...
    installNavigationHierarchy(runtime);
    runtime.engine.subscribe((event) => events.push(event));
    runtime.camera.position.set(0, 0, 10);
    runtime.camera.lookAt(runtime.positions.get('earth')!);
  ...
  it.each(['interaction', 'pinch', 'transition'] as const)(
    'efface la cible et la sélection sorties du viewport après %s',
    (source) => {
      const runtime = createRuntime();
      const events: UniverseEngineEvent[] = [];
      ...
      runtime.camera.updateMatrixWorld();
      runtime.positions.get('earth')!.set(20, 0, 0);
      runtime.access.targetId = 'earth';
      runtime.access.selectedId = 'earth';
      runtime.engine.subscribe((event) => events.push(event));

      runtime.access.handleCameraSettled(10, source);

      expect(runtime.access.targetId).toBeNull();
      expect(runtime.access.selectedId).toBeNull();
      expect(runtime.controller.releaseTarget).toHaveBeenCalledOnce();
      expect(events).toEqual([
        { type: 'camera-changed', zoom: 10 },
  ...
  it('attend le changement de cible d’échelle pendant la notification synchrone de molette', () => {
    const runtime = createRuntime();
    ...
    runtime.access.handleCameraSettled(10, 'zoom');

    expect(runtime.access.targetId).toBe('earth');
    expect(runtime.controller.releaseTarget).not.toHaveBeenCalled();
  });

  it('ne déduit pas une sortie de champ pendant un travelling ou la vue terrestre', () => {
    const runtime = createRuntime();
    ...
    runtime.controller.isTransitioning = true;
    runtime.access.handleCameraSettled(10, 'interaction');
    runtime.controller.isTransitioning = false;
    runtime.controller.observerPresentationActive = true;
    runtime.access.handleCameraSettled(10, 'zoom');
  ...
  it('conserve une cible dont la position ne peut pas encore être résolue', () => {
    const runtime = createRuntime();
    ...
    expect(runtime.access.targetId).toBe('unknown');
    expect(runtime.controller.releaseTarget).not.toHaveBeenCalled();
  });
...
interface FakeUniverseScene {
  ...
  readonly prewarmSceneMaterials: ReturnType<typeof vi.fn>;
  readonly prepareTempelFilamentSpineCatalog: ReturnType<typeof vi.fn>;
...
interface FakeCameraController {
  readonly controls: { target: THREE.Vector3; minDistance: number; maxDistance: number };
  ...
  readonly leaveObserver: ReturnType<typeof vi.fn>;
  ...
  readonly cancelInwardZoom: ReturnType<typeof vi.fn>;
  ...
  readonly adoptReferenceFrame: ReturnType<typeof vi.fn>;
...
function createRuntime(): Runtime {
  ...
  const controller: FakeCameraController = {
    controls: { target: new THREE.Vector3(), minDistance: 1.5, maxDistance: 18_000 },
    viewDirection: new THREE.Vector3(0.4, -0.2, -0.9),
    focusOn: vi.fn(),
  ...
  const scene: FakeUniverseScene = {
    ...
    prewarmSceneMaterials: vi.fn(async () => true),
    prepareTempelFilamentSpineCatalog: vi.fn(async () => ({
      geometryPreparationMs: 0,
      sceneInstallationMs: 0,
    })),
