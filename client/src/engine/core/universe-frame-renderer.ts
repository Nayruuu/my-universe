...
export interface UniverseFrameRendererBindings {
  ...
  setLabelsTransitioning(transitioning: boolean): void;
  renderLabels(
    camera: THREE.Camera,
...
export class UniverseFrameRenderer {
  ...
  constructor(
    ...
    this.readWorldPosition = (objectId, target) =>
      this.bindings.isObserverModeActive() && !this.bindings.isObserverSkyObject(objectId)
        ? null
        : this.bindings.getWorldPosition(objectId, target);
  }
  ...
  public render(
    ...
    if (this.bindings.labelsAllowed()) {
      ...
      const stellarNeighborhoodReveal = this.bindings.isObserverModeActive()
        ? 1
