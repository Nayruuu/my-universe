...
import { EarthObserverDeparture, EARTH_OBSERVER_DEPARTURE_EVENT } from './earth-observer-departure';
...
export class CameraController {
  ...
  private readonly observerDeparture: EarthObserverDeparture;
  private readonly mapFieldOfView: number;
  ...
  constructor(
    ...
    private readonly onCameraChanged: (distance: number) => void = () => undefined,
  ) {
    ...
    this.mapFieldOfView = camera.fov;
    this.observerDeparture = new EarthObserverDeparture(camera, this.controls.target, (detail) =>
      window.dispatchEvent(new CustomEvent(EARTH_OBSERVER_DEPARTURE_EVENT, { detail })),
    );
  ...
  public get viewDirection(): THREE.Vector3 {
    const direction = this.controls.target.clone().sub(this.camera.position);

    return direction.lengthSq() > Number.EPSILON ? direction.normalize() : direction.set(0, 0, -1);
  }
  ...
  public leaveObserver(position: THREE.Vector3, earth: SpaceObject, duration?: number): void {
    this.observerControl.deactivate();
    this.cancelFocus();
    this.targetTracker.track(position);
    this.controls.minDistance = getMinimumNavigationDistance(earth);
    this.controls.enabled = true;
    this.setTargetInteractionActive(true);
    this.observerDeparture.start(position, getFocusDistance(earth), this.mapFieldOfView, duration);
    if (!this.observerDeparture.active) {
      this.handleTransitionCompleted();
    }
  }
  ...
  public follow(
    ...
    if (this.observerDeparture.active) {
      this.observerDeparture.retarget(position);
      this.targetTracker.track(position);
  ...
  public update(deltaSeconds: number, time?: UniverseTime): void {
    if (this.observerDeparture.active) {
      const completed = this.observerDeparture.update(deltaSeconds);

      this.onCameraChanged(this.distanceToTarget);
      if (completed) {
        this.controls.update();
        this.handleTransitionCompleted();
      }
  ...
  public shiftTrackedPosition(originShift: THREE.Vector3): void {
    this.observerDeparture.shiftOrigin(originShift);
  ...
  private startTargetTransition(
    ...
    this.transitionController.start(endCamera, position, {
      ...
      logarithmicDistance: true,
      acquireTarget:
        this.distanceToTarget > Number.EPSILON &&
        travelDistance > Math.max(this.distanceToTarget, distance) * 0.25 &&
        this.camera.position.distanceTo(position) > distance * 4,
    });
  }
