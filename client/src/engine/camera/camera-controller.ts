...
export class CameraController {
  ...
  public follow(
    ...
    if (this.orbitInteractionActive || suppressedDistanceGuide) {
      ...
      if (!this.distanceGuidedTargetActive) {
        ...
        // Rebase the tracker on the pivot the user actually left on screen. Its logical baseline
        // may already be the destination object after a LOD hand-off or a suppressed drag; without
        // this rebase, the first guided frame reapplies that whole offset at once.
      ...
      // Finish the current pivot convergence before returning to ordinary tracking. Dropping the
      // guide as soon as its distance interval ended left a residual centre-to-Sun offset which
      // the target tracker then applied in one frame.
      this.distanceGuidedTarget.copy(position);
    } else {
  ...
  public adoptReferenceFrame(position: THREE.Vector3, object: SpaceObject): void {
    ...
    this.controls.minDistance = getMinimumNavigationDistance(object);
    this.setTargetInteractionActive(true);
    // A logical LOD hand-off must not replace the already smoothed Galactic pivot with the
    // destination pivot. The distance guide will keep converging on `position` over subsequent
    // frames; tracking that raw destination here made the next follow() compensate the whole
    // remaining offset in one frame and produced a visible sideways camera jump.
  ...
  private updateGuidedView(deltaSeconds: number): void {
    ...
    if (this.viewElevationGuideMode === 'distance') {
