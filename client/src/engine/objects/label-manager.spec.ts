...
describe('LabelManager', () => {
  ...
  it('gère son cycle de vie, ses dimensions, sa qualité et ses zones de clic', () => {
    ...
    manager.setWheelTargetObject('temporary');
    manager.setWheelTargetObject('temporary');
    manager.setWheelTargetObject(null);
  ...
  it('fond les labels du voisinage local tout en conservant les repères actifs', () => {
    ...
    painted.clear();
    manager.render(camera, reader, 2, null, 1_500, 0.8);
    expect(painted.get('earth')).toBeCloseTo(0.48030913, 8);
    expect(painted.get('kepler-b')).toBeCloseTo(0.48030913, 8);
    expect(painted.get('sirius')).toBe(0.8);
    expect(painted.get('sun')).toBe(1);

    painted.clear();
    manager.render(camera, reader, 2, null, 1_750, 0.3);
    expect(painted.get('sirius')).toBe(0.3);
    expect(painted.get('sun')).toBe(1);
    expect(painted.has('earth')).toBe(false);
    expect(painted.has('kepler-b')).toBe(false);
    expect(access.hitRegions.map(({ objectId }) => objectId).sort()).toEqual(['sirius', 'sun']);
  ...
  it('allège les lunes hors contexte et conserve leur sélection, leur survol et leur système ciblé', () => {
    const context = installContext();
    const titan = { ...createLabelObject('titan', 'moon'), parentId: 'saturn' };
    const europa = { ...createLabelObject('europa', 'moon'), parentId: 'jupiter' };
    const manager = new LabelManager(document.createElement('div'), [titan, europa], 'high');
    const access = manager as unknown as LabelManagerAccess;
    const camera = cameraForLabels();
    const reader = (id: string, target: THREE.Vector3): THREE.Vector3 =>
      target.set(id === 'titan' ? -0.5 : 0.5, 0, 0);
    const visibleIds = () => access.hitRegions.map(({ objectId }) => objectId);

    manager.resize(800, 400);
    manager.render(camera, reader, 1, 'sun', 1_000, 1, 1, 'sun');
    expect(visibleIds()).toEqual([]);

    manager.render(camera, reader, 1, 'saturn', 1_500, 1, 1, 'sun');
    expect(visibleIds()).toEqual(['titan']);
    expect(context.font).toContain('500 11px');

    manager.render(camera, reader, 1, 'earth', 2_000, 1, 1, 'jupiter');
    expect(visibleIds()).toEqual(['europa']);

    manager.render(camera, reader, 1, 'titan', 2_500, 0, 1, 'sun');
    expect(visibleIds()).toEqual(['titan']);
    expect(context.font).toContain('600 13px');

    manager.setHoveredObject('europa');
    manager.render(camera, reader, 1, 'sun', 3_000, 0, 1, 'sun');
    expect(visibleIds()).toEqual(['europa']);
    manager.dispose();
  });
