...
export class LabelCanvasPainter {
  ...
  public drawLabel(
    ...
    if (wheelTarget) {
      this.drawWheelTargetCue(rectangle, opacity, now);
    }
  ...
  private drawWheelTargetCue(rectangle: ScreenRectangle, opacity: number, now: number): void {
    const previousGlobalAlpha = this.context.globalAlpha;
    const pulse = 0.52 + (Math.sin(now / 180) + 1) * 0.14;
    const centerY = (rectangle.top + rectangle.bottom) / 2;
    const cueX = rectangle.right + 4;

    this.context.globalAlpha = clampOpacity(opacity * pulse);
    this.context.beginPath();
    this.context.moveTo(cueX, centerY - 3.5);
    this.context.lineTo(cueX + 4, centerY);
    this.context.lineTo(cueX, centerY + 3.5);
    this.context.strokeStyle = 'rgba(158, 224, 250, 0.96)';
