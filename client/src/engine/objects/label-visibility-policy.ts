...
export function getMaximumExoplanetHostLabelPoolRank(
  quality: GraphicQuality,
  density: LabelDensity,
): number {
  return Math.max(
    ...MAXIMUM_EXOPLANET_HOST_LABEL_RANKS[quality].map((rank) => scaleLabelLimit(rank, density)),
  );
}
...
export function getLabelPriority(object: LabelObject, lodLevel: number): number {
  ...
  if (
    lodLevel === 0 &&
    object.id !== 'moon' &&
    (object.type === 'moon' || object.type === 'asteroid' || object.type === 'comet')
  ) {
    return 3_000;
  }
  const catalogRecordIndex = object.metadata?.['catalogRecordIndex'];
