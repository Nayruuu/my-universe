...
describe('LabelCanvasPainter', () => {
  it('mesure et dessine toutes les variantes de cartouches et d’ancres', () => {
    ...
    const cueLineCount = context.lineTo.mock.calls.length;

    painter.drawLabel(catalog, catalogRectangle, false, true, -1, 0.5, true, 360);
    expect(context.lineTo).toHaveBeenCalledTimes(cueLineCount + 2);
    expect(context.strokeStyle).toBe('rgba(158, 224, 250, 0.96)');
    expect(context.globalAlpha).toBe(1);
  });
