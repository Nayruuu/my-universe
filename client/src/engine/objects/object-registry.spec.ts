...
describe('ObjectRegistry', () => {
  ...
  it('ancre une étoile hôte héliocentrique au Soleil et son exoplanète à cette étoile', () => {
    ...
    root.updateMatrixWorld(true);
    const transitioningScale = calculateStellarNeighborhoodSceneScale(1_400, 736.02);

    expect(access.stellarNeighborhoodRoot.scale.x).toBeCloseTo(transitioningScale.radialScale, 8);
    expect(access.stellarNeighborhoodRoot.scale.x).toBeGreaterThan(0.2);
    expect(access.stellarNeighborhoodRoot.scale.x).toBeLessThan(0.8);
  ...
  it('diffère les géométries orbitales et les libère hors des échelles solaires', () => {
    ...
    registry.updateReferenceFrameScale(1_100);
