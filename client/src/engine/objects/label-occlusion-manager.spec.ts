...
describe('LabelOcclusionManager', () => {
  it('collecte uniquement les disques apparents capables de masquer un label', () => {
    ...
    occlusion.setObjects([venus]);
    occlusion.collect(new THREE.OrthographicCamera(-1, 1, 1, -1, 0.1, 100), reader, 400, 300);
    expect(occlusion.occluderCount).toBe(0);

    occlusion.setObjects([venus, missingBody, tinyBody, outsideBody, zeroBody, region]);
    occlusion.collect(camera, reader, 400, 300);
  ...
  it('masque les objets plus lointains sans cacher le disque lui-même ni les objets proches', () => {
    ...
    venus.visual.visualRadius = 2;
    occlusion.setObjects([venus, behindStar, frontStar]);
    occlusion.collect(
      camera,
      positionReader(
        new Map([
  ...
  it('masque aussi un label sélectionné derrière une étoile ou un trou noir', () => {
    ...
    occlusion.setObjects([sun, blackHole, selectedPlanet]);
    occlusion.collect(
      camera,
      positionReader(
        new Map([
