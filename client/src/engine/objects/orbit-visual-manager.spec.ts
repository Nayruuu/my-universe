...
describe('OrbitVisualManager', () => {
  ...
  it('fond les orbites locales sans rupture au passage du seuil stellaire', () => {
    ...
    manager.update(state({ lodLevel: 1, stellarNeighborhoodReveal: 0.8 }));
    const earth = orbitLine(fixture.root, 'earth');
    const candidate = orbitLine(fixture.root, 'candidate');

    expect(earth.material.opacity).toBeCloseTo(0.29779166, 8);
    expect(candidate.material.opacity).toBeCloseTo(0.29779166, 8);
    expect(earth.userData['overviewReveal']).toBeCloseTo(0.48030913, 8);
    expect(candidate.userData['overviewReveal']).toBeCloseTo(0.48030913, 8);

    manager.update(state({ lodLevel: 2, stellarNeighborhoodReveal: 0.8 }));
    ...
    manager.update(state({ lodLevel: 2, stellarNeighborhoodReveal: 0.3 }));
