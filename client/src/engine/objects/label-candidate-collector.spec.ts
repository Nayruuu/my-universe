...
describe('LabelCandidateCollector', () => {
  it('conserve la sélection et les repères tout en filtrant les objets invisibles', () => {
    ...
    expect(candidates[0]).toMatchObject({ worldX: 0.2, worldY: 0, worldZ: 0 });
    ...
    expect(reader).toHaveBeenCalledTimes(7);
  });
  ...
  it('vérifie la visibilité dynamique des catalogues stellaires déjà filtrés par rang', () => {
    ...
    const positions = new Map([
      ['hyg-1', new THREE.Vector3()],
      ['nea-host', new THREE.Vector3(1, 0, 0)],
    ]);

    expect(
      collector.collect({
        objects: [catalogStar, host],
        transientObject: null,
        camera: cameraForLabels(),
        readWorldPosition: positionReader(positions),
        lodLevel: 1,
        selectedId: null,
        quality: 'high',
  ...
  it('réutilise les candidats alloués entre deux images', () => {
    const collector = new LabelCandidateCollector();
    const object = createLabelObject('earth', 'planet');
    const options = {
      objects: [object],
      transientObject: null,
      camera: cameraForLabels(),
      readWorldPosition: positionReader(new Map([['earth', new THREE.Vector3()]])),
      lodLevel: 1,
      selectedId: null,
    ...
    const first = collector.collect(options)[0];
    const second = collector.collect(options)[0];
...
function positionReader(positions: ReadonlyMap<string, THREE.Vector3>) {
  return vi.fn((objectId, target) => {
