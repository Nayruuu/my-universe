...
import {
  ...
  type DeferredProceduralTextureFactory,
...
import { createDeferredObservedShape } from './deferred-observed-shape';
...
export function requestCelestialLodTextures(lod: CelestialLodRepresentation): number {
  ...
  for (const texture of lod.deferredTextures) {
    ...
    const proceduralFactory: unknown = texture.userData[DEFERRED_PROCEDURAL_TEXTURE_FACTORY];
    const image: unknown = texture.image;

    if (typeof source === 'string' && image instanceof HTMLImageElement) {
      image.src = source;
      requestedTextures += 1;
    } else if (typeof proceduralFactory === 'function') {
      texture.image = (proceduralFactory as DeferredProceduralTextureFactory)();
      texture.needsUpdate = true;
      delete texture.userData[DEFERRED_PROCEDURAL_TEXTURE_FACTORY];
      requestedTextures += 1;
    }
  }
...
export function createCelestialVisual(
  ...
  const bodyShape = object.physical?.shape;
  const [shapeScaleX, shapeScaleY, shapeScaleZ] = calculateTriaxialBodyScale(bodyShape);

  body.scale.set(
    visualRadius * shapeScaleX,
    visualRadius * shapeScaleY,
    visualRadius * shapeScaleZ,
  );
  if (bodyShape) {
    body.userData['shapeDimensionsKm'] = [...bodyShape.dimensionsKm];
    body.userData['shapeScientificConfidence'] = bodyShape.scientificConfidence;
    body.userData['shapeSource'] = bodyShape.source;
  }
  const nearMaterials: ManagedLodMaterial[] = [manageMaterial(bodyMaterial)];
  const observedShapeDefinition = getObservedBodyShapeDefinition(object.id);
  let rotatingBody: THREE.Object3D = body;
  let deferredObservedShape = null;

  if (observedShapeDefinition || bodyShape) {
    const rotationRoot = new THREE.Group();

    rotationRoot.name = `${object.id}-rotation-root`;
    rotationRoot.add(body);
    nearRoot.add(rotationRoot);
    rotatingBody = rotationRoot;
    if (observedShapeDefinition) {
      deferredObservedShape = createDeferredObservedShape({
        objectId: object.id,
        rotatingRoot: rotationRoot,
        fallbackBody: body,
        visualRadius,
        nearMaterials,
      });
  ...
  const cometActivity =
    object.type === 'comet' && object.cometActivity
      ? createCometActivityVisual(object, quality, assets.glowTexture, nearMaterials)
      : null;

  if (cometActivity) {
    nearRoot.add(cometActivity.root);
  }
  ...
  const hitRadius = Math.max(
    visualRadius * Math.max(shapeScaleX, shapeScaleY, shapeScaleZ) * 1.45,
    object.type === 'star' ? 2.2 : 0.72,
  );
  ...
  if (object.visual.hasRings) {
    ...
    if (rotatingBody === body) {
      body.add(rings);
    } else {
      rings.scale.setScalar(visualRadius);
      rotatingBody.add(rings);
    }
  ...
  return {
    ...
    lod: {
      ...
      ...(deferredObservedShape ? { deferredResources: [deferredObservedShape] } : {}),
      nearBlend: 0,
      visibilityBlend: 0,
      farAlpha: 0,
      farBaseOpacity,
