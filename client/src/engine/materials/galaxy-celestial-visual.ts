...
export function createGalaxyCelestialVisual(
  ...
  if (isMilkyWay) {
    ...
    // Retain the sizing metadata used by labels and selection, never a visible image.
    halo.material.opacity = 0;
    halo.material.colorWrite = false;
    halo.material.userData['visualStyle'] = 'transparent-galaxy-scale-proxy';
  }
  halo.visible = false;
  root.add(halo);
  ...
  const pickables = hitTarget ? [hitTarget] : [halo];
  ...
  return {
    ...
    lod: {
      ...
      continuousVolume: volume ?? undefined,
      nearBlend: 0,
      visibilityBlend: 0,
      farAlpha: 0,
