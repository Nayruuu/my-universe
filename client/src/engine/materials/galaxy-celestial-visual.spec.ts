...
describe('galaxy celestial visual', () => {
  ...
  it('compose un volume continu et un proxy de taille invisible dans le même contrat LOD', () => {
    ...
    expect(visual.lod.nearMaterials).toHaveLength(1);
    expect(visual.lod.continuousVolume).toBeDefined();
    expect(halo?.material.opacity).toBe(0);
    expect(halo?.material.colorWrite).toBe(false);
  ...
  it('réserve le volume spécialisé de la Voie lactée et un proxy de sélection invisible', () => {
    ...
    expect(visual.lod.continuousVolume).toBeUndefined();
