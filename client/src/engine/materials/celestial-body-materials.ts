...
export function createCelestialBodyMaterial(
  ...
  const illustrativeShadowFill =
    ...
    (object.type === 'exoplanet' ||
      ...
      object.id === 'uranus' ||
      object.id === 'neptune');
  const observedGrayscaleTint = texture?.userData['colorTreatment'] === 'grayscale';
  const isMoon = object.id === 'moon';
  const materialParameters: THREE.MeshStandardMaterialParameters = {
    color: texture && !observedGrayscaleTint ? (isMoon ? 0xd8d4ca : 0xffffff) : primaryColor,
    map: texture,
    bumpMap: lunarRelief,
    bumpScale: lunarRelief ? 0.032 : 0,
    displacementMap: lunarRelief,
    displacementScale: lunarRelief ? 0.012 : 0,
    displacementBias: lunarRelief ? -0.006 : 0,
    roughness: object.id === 'earth' ? 0.68 : isMoon ? 1 : 0.88,
    metalness: 0,
    ...
    emissiveMap: earthNightLights ?? (lunarRelief || illustrativeShadowFill ? texture : null),
  ...
  if (texture?.userData['visualStyle'] === 'illustrative-nasa-vtad-atmosphere-map') {
    material.userData['visualStyle'] = 'illustrative-atmospheric-snapshot';
  }
  const textureConfidence: unknown = texture?.userData['scientificConfidence'];

  if (typeof textureConfidence === 'string') {
    material.userData['scientificConfidence'] = textureConfidence;
  }
  if (lunarRelief) {
    material.userData['reliefScale'] = 'visually-exaggerated';
    material.userData['silhouetteReliefScale'] = 'lola-derived-visually-exaggerated';
    material.userData['shadowFill'] = 'illustrative-earthshine-like-fill';
    material.userData['visualStyle'] = 'observed-lro-regolith-with-lola-relief';
  }
  if (illustrativeShadowFill) {
    material.userData['shadowFill'] = 'illustrative';
  }
  if (observedGrayscaleTint) {
    material.userData['surfaceTintConfidence'] = 'illustrative';
  }
    ...
    for (const value of Object.values(managed.material)) {
      if (
        ...
        (typeof value.userData[DEFERRED_TEXTURE_SOURCE] === 'string' ||
          typeof value.userData[DEFERRED_PROCEDURAL_TEXTURE_FACTORY] === 'function')
      ) {
        textures.add(value);
      }
    }
