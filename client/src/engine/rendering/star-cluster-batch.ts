...
interface ClusterRepresentation {
  ...
  /** Number of pickable Gaia sources currently exposed by the render batch. */
...
function updatePickableIndices(representation: ClusterRepresentation): void {
  ...
  const nextVisibleDrawCount = representation.points.visible
    ? representation.points.geometry.drawRange.count
    : 0;

  // Gaia selection shares the same source order as the GPU draw range. Updating only the
  // changed suffix avoids clearing and restoring tens of thousands of bytes every frame while
  // preserving the exact clickability boundary when an LOD or fade changes.
  if (nextVisibleDrawCount > representation.visibleDrawCount) {
    visibleIndices.fill(1, representation.visibleDrawCount, nextVisibleDrawCount);
  } else if (nextVisibleDrawCount < representation.visibleDrawCount) {
    visibleIndices.fill(0, nextVisibleDrawCount, representation.visibleDrawCount);
  }
  representation.visibleDrawCount = nextVisibleDrawCount;
}
