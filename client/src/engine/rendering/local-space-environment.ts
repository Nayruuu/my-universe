...
export class LocalSpaceEnvironment {
  ...
  public async prewarmPanorama(target: TexturePrewarmTarget): Promise<boolean> {
    if (!(await this.ensurePanorama()) || this.disposed) {
      return false;
    }
...
async function loadLocalPanorama(url: string): Promise<THREE.Texture> {
  ...
  if (!response.ok) {
    throw new Error(`Panorama indisponible (${response.status}).`);
  }
  const blob = await response.blob();
  let bitmap: ImageBitmap;

  try {
    // Bake TextureLoader's default flip and unpremultiplied pixels into the async bitmap;
    // ImageBitmap uploads ignore Texture.flipY and Texture.premultiplyAlpha.
    ...
    // Keep the ordinary image loader on browsers unable to decode this format as a bitmap.
    return new THREE.TextureLoader().loadAsync(url);
  }
  const texture = new THREE.Texture(bitmap);
  const releaseBitmap = (): void => {
    texture.removeEventListener('dispose', releaseBitmap);
    bitmap.close();
  };

  texture.addEventListener('dispose', releaseBitmap);
