...
const DISC_LAYER_STRENGTHS = [1, 0.14, 0.09] as const;
...
export class MilkyWayVolumeVisual {
  ...
  private readonly discMaterials = DISC_LAYER_STRENGTHS.map((strength, index) =>
    createDiscMaterial(strength, index),
  );
  ...
  constructor() {
    ...
    this.root.userData['samplingTechnique'] = 'grazing-angle-mip-bias-with-anisotropic-filtering';
  ...
  public get atlasTexture(): THREE.Texture | null {
    return this.atlas;
  }
...
function createDiscMaterial(strength: number, layerIndex: number): THREE.ShaderMaterial {
  const profile = getMilkyWayCinematicProfile('medium');
  const additive = layerIndex > 0;
  ...
  return new THREE.ShaderMaterial({
    uniforms: {
      ...
      layerSamplingBias: { value: layerIndex === 0 ? 0 : 0.5 + layerIndex * 0.18 },
    },
    vertexShader: `
      varying vec2 vUv;
      varying vec2 viewParallax;
      varying float viewFacing;
      ...
      void main() {
        ...
        float facing = max(abs(dot(directionToCamera, planeNormal)), 0.22);
        viewFacing = clamp(facing, 0.0, 1.0);
        viewParallax = vec2(
    ...
    fragmentShader: `
      ...
      uniform float layerSamplingBias;
      varying vec2 vUv;
      varying vec2 viewParallax;
      varying float viewFacing;
      ...
      void main() {
        ...
        float samplingBias = layerSamplingBias + (1.0 - viewFacing) * 5.0;
        vec3 atlasColor = texture2D(atlas, warpedUv, samplingBias).rgb;
        vec3 frontColor = texture2D(atlas, frontUv, samplingBias).rgb;
        vec3 rearColor = texture2D(atlas, rearUv, samplingBias).rgb;
