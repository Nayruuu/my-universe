...
describe('createExoplanetHostVisual', () => {
  it('construit les photosphères GPU et les index des hôtes NASA rendus', () => {
    ...
    expect(visual.points.userData).toMatchObject({
      ...
      pointSizeModel: 'magnitude-with-bounded-camera-proximity-and-optical-magnification',
    });
    expect(visual.points.material.vertexShader).toContain('stellarPointProximityGrowth');
    expect(visual.points.material.vertexShader).toContain('stellarPointProjectionGrowth');
    expect(visual.points.material.vertexShader).toContain('stellarPointMaximumGrowth = 2.8');
  });
});
