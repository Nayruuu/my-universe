...
describe('ExoplanetHostBatch', () => {
  ...
  it('keeps the catalogue index and selected marker when Gaia owns the rendered field', () => {
    const registry = new ExoplanetCatalogRegistry(catalog(), new CoordinateSystem());
    const batch = new ExoplanetHostBatch(registry, 'high', false);
    const planetId = registry.getPlanetObjectId(0);

    batch.updateLod(2, 10);
    expect(batch.points.visible).toBe(false);
    expect(batch.points.userData['fieldRendering']).toBe('suppressed-in-favor-of-gaia');
    expect(batch.points.userData['activeCount']).toBe(0);
    expect(batch.visibleCount).toBe(0);
