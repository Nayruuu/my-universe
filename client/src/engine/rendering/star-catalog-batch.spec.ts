...
describe('StarCatalogBatch', () => {
  ...
  it('retire le catalogue exact avant la représentation galactique', () => {
    ...
    expect(batch.activeDetail.visible).toBe(false);
    expect(batch.activeDetail.userData['objectId']).toBe('hyg-6000');
    batch.dispose();
  });

  it('masque aussi les repères actifs avant de transformer le voisinage stellaire', () => {
    ...
    expect(batch.activeHalo.visible).toBe(true);
    expect(batch.isObjectVisibleForLabels('hyg-1')).toBe(true);
    expect(batch.isObjectVisibleForLabels('hyg-2')).toBe(true);
    ...
    expect(batch.selectionPoint.userData['objectId']).toBe('hyg-1');
    expect(batch.activeDetail.visible).toBe(false);
    expect(batch.activeDetail.userData['objectId']).toBe('hyg-2');
    expect(batch.activeHalo.visible).toBe(false);
    expect(batch.activeCore.visible).toBe(false);
    expect(batch.isObjectVisibleForLabels('hyg-1')).toBe(true);
    expect(batch.isObjectVisibleForLabels('hyg-2')).toBe(true);
  ...
  it('conserve les noms HYG en mode observateur sans réafficher leur champ de points', () => {
    const batch = new StarCatalogBatch(
      new StarCatalogRegistry(createCatalog(2), new CoordinateSystem()),
      'high',
      false,
    );

    batch.updateLod(2, 10, undefined, STELLAR_NEIGHBORHOOD_REVEAL_END, true);

    expect(batch.points.visible).toBe(false);
    expect(batch.visibleCount).toBe(0);
    expect(batch.isObjectVisibleForLabels('hyg-1')).toBe(true);
    expect(batch.getPickables()).toEqual([batch.selectionPoint]);
    batch.dispose();
  });

  it('atténue sa frontière héliocentrique sans retirer les points pendant un trajet local', () => {
    const batch = createBatch(6_000);
    ...
    expect(batch.points.userData).toMatchObject({
      ...
      remoteVisibilityFloor: 0.12,
      catalogCompleteness: 'finite-heliocentric-bright-star-sample',
    });
  ...
  it('fait suivre le détail à la cible sans déplacer le marqueur de sélection', () => {
    const batch = createBatch(2);
    const firstPosition = batch.getWorldPosition('hyg-1')!;
    const secondPosition = batch.getWorldPosition('hyg-2')!;

    batch.select('hyg-1');
    batch.focus('hyg-2');

    expect(batch.selectionPoint.userData['objectId']).toBe('hyg-1');
    expect(batch.selectionPoint.position.distanceTo(firstPosition)).toBeLessThan(1e-6);
    expect(batch.activeDetail.userData).toMatchObject({
      objectId: 'hyg-2',
      activation: 'navigation-target',
    });
    expect(batch.activeDetail.position.distanceTo(secondPosition)).toBeLessThan(1e-6);
    ...
    expect(batch.activeDetail.position.distanceTo(firstPosition)).toBeLessThan(1e-6);

    batch.focus('missing');
    expect(batch.activeDetail.userData['objectId']).toBe('hyg-1');
    batch.dispose();
  });
  ...
  it('garde la photosphère ciblée lisible au zoom habituel sans accélérer le fondu du champ', () => {
    const batch = createBatch(2);

    batch.select('hyg-1');
    batch.focus('hyg-2');
    batch.updateLod(2, 10, undefined, 1_400);
    expect(batch.points.visible).toBe(false);
    expect(batch.points.userData['stellarNeighborhoodReveal']).toBe(0);
    expect(batch.selectionPoint.visible).toBe(true);
    expect(batch.activeDetail.visible).toBe(true);
    ...
    batch.updateLod(2, 10, undefined, 800);
    const passiveOpacity = batch.points.material.uniforms['catalogOpacity']!.value as number;

    batch.select('hyg-1');
    batch.focus('hyg-2');
    batch.updateLod(2, 10, undefined, 800);

    expect(batch.points.userData['stellarNeighborhoodReveal']).toBeCloseTo(0.007582052, 8);
    expect(batch.points.material.uniforms['catalogOpacity']!.value).toBeCloseTo(passiveOpacity, 8);
    expect(batch.activeCore.visible).toBe(true);
    expect(batch.activeCore.material.opacity).toBeCloseTo(0.125697165, 8);
    expect(batch.activeCore.material.uniforms['layerOpacity']!.value).toBeCloseTo(0.125697165, 8);
    expect(batch.activeHalo.material.uniforms['pointSize']!.value).toBeGreaterThan(48);
    expect(batch.activeDetail.userData['objectId']).toBe('hyg-2');

    batch.updateLod(2, 10, undefined, 520);
    expect(batch.activeCore.material.opacity).toBeCloseTo(0.28, 8);
    expect(batch.points.userData['stellarNeighborhoodReveal']).toBeLessThan(0.15);
  ...
  it('met à jour le buffer partagé et les détails actifs avec le mouvement propre', () => {
    const catalog = createCatalog(1);

    catalog.velocitiesParsecPerYear[0] = 0.01;
    const registry = new StarCatalogRegistry(catalog, new CoordinateSystem());
    const batch = new StarCatalogBatch(registry);

    batch.select('hyg-1');
    batch.focus('hyg-1');
    const initialSelection = batch.selectionPoint.position.clone();
    const initialDetail = batch.activeDetail.position.clone();
    const positionAttribute = batch.points.geometry.getAttribute(
      'position',
    ) as THREE.BufferAttribute;
    const initialVersion = positionAttribute.version;
    const time = { julianDay: 2_451_545 + 100 * 365.25 };

    expect(batch.updateTime(time)).toBe(true);
    expect(positionAttribute.version).toBeGreaterThan(initialVersion);
    expect(batch.selectionPoint.position).not.toEqual(initialSelection);
    expect(batch.activeDetail.position).not.toEqual(initialDetail);
    expect(batch.points.userData).toMatchObject({
      requestedEpochJulianDay: time.julianDay,
      appliedMotionEpochJulianDay: time.julianDay,
      stellarMotionDomainStatus: 'within-model-domain',
    });
    expect(batch.updateTime(time)).toBe(false);
    batch.dispose();
  });
});
