...
export class GalacticTransitionLayer {
  ...
  constructor(
    ...
    this.milkyWay.userData['colorStructure'] =
      'temperature-structured-sapphire-cyan-ivory-amber-red-stars-and-magenta-hii';
  ...
  public setQuality(quality: GraphicQuality): void {
    ...
    const densityCompensation = GALACTIC_DETAIL_QUALITY_DENSITY_COMPENSATION[quality];
  ...
  public update(frame: GalacticTransitionFrame): void {
    ...
    const targetStellarHaloOpacity = transitionVisible
    ...
    this.milkyWayMaterial.uniforms['flythroughContext']!.value = calculateGalacticFlythroughContext(
      frame.cameraDistance,
    );
  ...
  private synchronizeBackdropWithObserver(observerPosition?: Vector3Like): void {
    ...
    this.extragalacticBackground.setObserverPosition(this.backdrop.position);
  }
...
const GALACTIC_FLYTHROUGH_CONTEXT_FADE_IN_START = 70;
const GALACTIC_FLYTHROUGH_CONTEXT_FADE_IN_END = 2_200;
const GALACTIC_FLYTHROUGH_CONTEXT_FADE_START = 3_600;
const GALACTIC_FLYTHROUGH_CONTEXT_FADE_END = 6_200;
...
interface MilkyWayGeometryTemplate extends BackdropGeometryTemplate {
  readonly softness: Float32Array;
  readonly flythrough: Float32Array;
}

let backdropGeometryTemplate: BackdropGeometryTemplate | null = null;
let milkyWayGeometryTemplate: MilkyWayGeometryTemplate | null = null;
...
/**
 * Keeps the traversal tracers out of planetary views, then reveals them across a broad logarithmic
 * distance range. Their spatial proximity to the camera remains the local emphasis cue; the wide
 * inner ramp avoids switching the whole field on at one Solar-system scale boundary.
 */
export function calculateGalacticFlythroughContext(cameraDistance: number): number {
  if (!Number.isFinite(cameraDistance)) {
    return 0;
  }
  const distance = Math.max(0, cameraDistance);
  const innerPresence =
    distance > 0
      ? smoothstep(
          Math.log(GALACTIC_FLYTHROUGH_CONTEXT_FADE_IN_START),
          Math.log(GALACTIC_FLYTHROUGH_CONTEXT_FADE_IN_END),
          Math.log(distance),
        )
      : 0;
  const outerPresence =
    1 -
    smoothstep(
      GALACTIC_FLYTHROUGH_CONTEXT_FADE_START,
      GALACTIC_FLYTHROUGH_CONTEXT_FADE_END,
      distance,
    );

  return innerPresence * outerPresence;
}
...
function createBackdropGeometryTemplate(): BackdropGeometryTemplate {
  ...
  const sapphire = new THREE.Color(0x4f82ff);
  const cyan = new THREE.Color(0x52d2ff);
  const ivory = new THREE.Color(0xffd7a3);
  const amber = new THREE.Color(0xff8a24);
  const red = new THREE.Color(0xff4038);
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (temperature < 0.14) {
      color.lerpColors(sapphire, cyan, temperature / 0.14);
    } else if (temperature < 0.31) {
      color.lerpColors(cyan, ivory, (temperature - 0.14) / 0.17);
    } else if (temperature < 0.64) {
      color.copy(ivory);
    } else if (temperature < 0.89) {
      color.lerpColors(ivory, amber, (temperature - 0.64) / 0.25);
    } else {
      color.lerpColors(amber, red, (temperature - 0.89) / 0.11);
    }
...
function createMilkyWayGeometry(): THREE.BufferGeometry {
  ...
  geometry.setAttribute('pointSize', new THREE.BufferAttribute(template.sizes, 1));
  geometry.setAttribute('pointAlpha', new THREE.BufferAttribute(template.alphas, 1));
  geometry.setAttribute('pointSoftness', new THREE.BufferAttribute(template.softness, 1));
  geometry.setAttribute('pointFlythrough', new THREE.BufferAttribute(template.flythrough, 1));

  return geometry;
}

function getMilkyWayGeometryTemplate(): MilkyWayGeometryTemplate {
  milkyWayGeometryTemplate ??= createMilkyWayGeometryTemplate();

  return milkyWayGeometryTemplate;
}

function createMilkyWayGeometryTemplate(): MilkyWayGeometryTemplate {
  ...
  const sapphire = new THREE.Color(0x477fff);
  const cyan = new THREE.Color(0x47d4ff);
  const ivory = new THREE.Color(0xffd49a);
  const amber = new THREE.Color(0xff791f);
  const red = new THREE.Color(0xff3b32);
  const magenta = new THREE.Color(0xff3294);
  ...
  return { positions, colors, sizes, alphas, softness, flythrough };
}
...
function createMilkyWayMaterial(): THREE.ShaderMaterial {
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      uniform float flythroughContext;
      ...
      void main() {
        ...
        float broadFlythroughProximity = 1.0 - smoothstep(320.0, 1500.0, distanceToCamera);
        float corridorFlythroughProximity = 1.0 - smoothstep(700.0, 2800.0, distanceToCamera);
        float nearPassageProximity = 1.0 - smoothstep(120.0, 900.0, distanceToCamera);
        float broadFlythroughEmphasis =
          broadFlythroughMask * flythroughContext * broadFlythroughProximity;
        float corridorFlythroughEmphasis =
          corridorFlythroughMask * flythroughContext * corridorFlythroughProximity;
        float nearPassageEmphasis =
          nearPassageMask * flythroughContext * nearPassageProximity;
        ...
        float nearPassageGrowth = clamp(3400.0 / distanceToCamera, 1.0, 26.0);
        ...
        float stellarExposure = 1.08
          ...
          + flythroughContext
            * max(broadFlythroughMask, max(corridorFlythroughMask, nearPassageMask))
            * 0.32;
        float qualityPointScale = sqrt(qualityDensityCompensation);
        float qualityColorGain = mix(
          1.0,
          1.12,
          clamp(qualityDensityCompensation - 1.0, 0.0, 1.0)
        );
        float stellarLuminance = dot(color, vec3(0.2126, 0.7152, 0.0722));
        vec3 chromaticColor = max(
          vec3(0.0),
          mix(vec3(stellarLuminance), color, 1.18)
        );
        starColor = chromaticColor * stellarExposure * qualityColorGain;
        ...
        float broadVisibility = mix(
          1.0,
          mix(0.12, 1.75, pow(broadFlythroughProximity, 1.7)),
          broadFlythroughMask * flythroughContext
        );
        float corridorVisibility = mix(
          1.0,
          flythroughContext * mix(0.075, 1.6, pow(corridorFlythroughProximity, 1.35)),
          corridorFlythroughMask
        );
        float nearPassageVisibility = mix(
          1.0,
          flythroughContext * mix(0.035, 2.55, pow(nearPassageProximity, 1.25)),
          nearPassageMask
        );
        starAlpha = pointAlpha
          * immersionVisibility
        ...
        float minimumPointSize = qualityPointScale
          * mix(
            0.7,
            1.25,
            flythroughEmphasis
        ...
        gl_PointSize = clamp(
          pointSize
            ...
            * perspectiveGrowth
            * qualityPointScale
            * mix(1.0, 1.45, starTrailStrength),
          minimumPointSize,
          30.0 * pixelRatio
        );
      }
