...
export class GalacticTransitionLayer {
  ...
  public setQuality(quality: GraphicQuality): void {
    ...
    this.milkyWayGeometry.setDrawRange(
      0,
      Math.min(
        this.performanceManager.getParticleCount(quality) * MILKY_WAY_PARTICLE_GROUP_SIZE[quality],
  ...
  public update(frame: GalacticTransitionFrame): void {
    ...
    const targetOpacity = detailStrength * detailTransitionPresence;
    ...
    this.dustField.update(milkyWayOpacity, milkyWaySceneScale.modelScale, frame.observerPosition);
