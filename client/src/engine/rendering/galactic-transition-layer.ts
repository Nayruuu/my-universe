...
export class GalacticTransitionLayer {
  ...
  constructor(
    ...
    this.milkyWay.userData['visualStyle'] = 'deterministic-stellar-detail-over-atlas';
  ...
  public update(frame: GalacticTransitionFrame): void {
    ...
    const detailStrength = frame.milkyWayAtlasReady
      ? MILKY_WAY_ATLAS_DETAIL_OPACITY
      : MILKY_WAY_FALLBACK_OPACITY;
    const targetOpacity = transitionVisible
      ? transition.detailOpacity * detailStrength * frame.galaxyRadiance
      : 0;
...
const MILKY_WAY_FALLBACK_OPACITY = 0.03;
const MILKY_WAY_ATLAS_DETAIL_OPACITY = 0.42;
...
function createMilkyWayGeometry(count: number): THREE.BufferGeometry {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const prominence = Math.pow(random(), 5.2);

    sizes[index] = 1.1 + prominence * 3.2;
    alphas[index] = 0.28 + random() * 0.38 + prominence * 0.28;
  }
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        gl_PointSize = max(
          0.75,
          pointSize * pixelRatio * stellarPointProximityGrowth(viewPosition.xyz)
        );
      }
    `,
    fragmentShader: `
      ...
      void main() {
        ...
        float halo = pow(1.0 - radius, 1.9);
        float core = 1.0 - smoothstep(0.0, 0.24, radius);
        float alpha = (halo * 0.72 + core * 0.62) * starAlpha * opacity;
        vec3 color = starColor * (0.82 + core * 1.35);
