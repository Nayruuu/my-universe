...
const DENSITY_SIZE = [64, 32, 64] as const;
...
export class MilkyWayDustField {
  ...
  constructor(source: THREE.BufferAttribute | THREE.InterleavedBufferAttribute) {
    ...
    this.points.userData = {
      ...
      maximumGrainRasterPixels: 5,
      maximumUnresolvedCloudRasterPixels: 240,
    };
  ...
  public setQuality(quality: GraphicQuality): void {
    const samples = PARTICLES_PER_CELL[quality];
  ...
  public update(opacity: number, modelScale: number, observerPosition?: Vector3Like): void {
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        float rasterDiameter = 5.0 * pixelRatio;
        ...
        dustAlpha = min(0.85, cloud * (0.55 + seed.z * 0.65) * sampleWeight)
        ...
        if (unresolvedCloud > 0.5) {
          ...
          // Their low surface brightness never turns a grain into a bright, enlarged star halo.
          float projectedCloud = cellSize * 2.8 * worldScale * focalPixels
            / max(-viewPosition.z, 0.001);
          rasterDiameter = min(240.0 * pixelRatio, projectedCloud);
          dustAlpha = 0.012 * cloud * support * innerSupport * (1.0 - step(0.5, sampleSeed));
