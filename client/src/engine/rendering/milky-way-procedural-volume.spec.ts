...
describe('MilkyWayProceduralVolume', () => {
  it('encode un champ de densité RGBA illustratif et adapte le lancer de rayons', () => {
    ...
    expect(volume.material.fragmentShader).toContain(
      'vec3 sapphireStars = vec3(0.035, 0.24, 1.32)',
    );
    expect(volume.material.fragmentShader).toContain('vec3 ionizedCyan = vec3(0.025, 0.72, 1.08)');
    expect(volume.material.fragmentShader).toContain('vec3 warmCore = vec3(1.34, 0.27, 0.025)');
    ...
    expect(volume.material.fragmentShader).toContain('float chromaGain = 1.1');
