...
describe('MilkyWayProceduralVolume', () => {
  it('encode un champ de densité RGBA illustratif et adapte le lancer de rayons', () => {
    ...
    expect(volume.mesh.userData).toMatchObject({
      ...
      structuralContrastTreatment:
        'localized-near-black-dust-extinction-separated-from-emissive-starlight',
      compositingTreatment: 'ray-marched-extinction-over-additive-stellar-detail',
      coreTreatment: 'compact-ivory-nucleus-inside-an-amber-bar-with-paired-dust-lanes',
      colorStructure:
      ...
      integratedLightTreatment:
        'continuous-unresolved-stellar-light-structured-into-arms-filaments-and-clumps',
      viewAdaptiveTreatment: 'crisp-exterior-arm-contrast-and-restrained-interior-integrated-light',
      observerResolutionTreatment:
        'nearby-unresolved-density-resolves-spatially-into-co-spatial-batched-stars',
      approachResolutionTreatment:
        'integrated-density-recedes-continuously-as-co-spatial-stellar-parallax-takes-over',
    ...
    expect(thickDiscEmission).toBeGreaterThan(midplaneEmission * 0.01);
    expect(thickDiscEmission).toBeLessThan(midplaneEmission * 0.06);
    ...
    expect(volume.material.fragmentShader).toContain('uniform vec3 volumeHalfExtents');
    expect(volume.material.fragmentShader).toContain('observerAuthoringDistance');
    expect(volume.material.fragmentShader).toContain('localIntegratedLightPresence');
    expect(volume.material.fragmentShader).toContain('uniform float integratedLightResolution');
    expect(volume.material.fragmentShader).toContain('float unresolvedVolumePresence');
    expect(volume.material.fragmentShader).toContain('smoothstep(180.0, 1050.0');
    expect(volume.material.fragmentShader).toContain('exteriorView');
    expect(volume.material.fragmentShader).toContain('sampler3D');
    expect(volume.material.fragmentShader).toContain('stellarEmission');
    expect(volume.material.fragmentShader).toContain('pow(densitySample.r, 1.52)');
    expect(volume.material.fragmentShader).toContain('youngStarTint');
    expect(volume.material.fragmentShader).toContain('youngArmStrength');
    expect(volume.material.fragmentShader).toContain('warmTint');
    expect(volume.material.fragmentShader).toContain('barStrength');
    expect(volume.material.fragmentShader).toContain('neutralStarlight');
    expect(volume.material.fragmentShader).toContain('vec3 sapphireStars = vec3(0.045, 0.28, 1.4)');
    expect(volume.material.fragmentShader).toContain('vec3 violetCloud');
    expect(volume.material.fragmentShader).toContain('vec3 coolIntegratedLight');
    expect(volume.material.fragmentShader).toContain('vec3 ionizedCyan = vec3(0.028, 0.82, 1.18)');
    expect(volume.material.fragmentShader).toContain('vec3 warmCore = vec3(1.42, 0.34, 0.035)');
    expect(volume.material.fragmentShader).toContain('compactNucleus');
    expect(volume.material.fragmentShader).toContain('dustOpticalDensity');
    expect(volume.material.fragmentShader).toContain('emissiveShare');
    expect(volume.material.fragmentShader).not.toContain('brownDust');
    expect(volume.material.fragmentShader).toContain('pinkNebula');
    expect(volume.material.fragmentShader).toContain('hiiStrength');
    expect(volume.material.fragmentShader).toContain('nebulaNoise');
    expect(volume.material.fragmentShader).toContain('chromaticLuminance');
    expect(volume.material.fragmentShader).toContain('float chromaGain = 1.18');
    ...
    expect(volume.mesh.renderOrder).toBe(4);
    expect(volume.material.blending).toBe(THREE.NormalBlending);
  ...
  it('masque le volume transparent, transmet sa radiance et libère ses ressources', () => {
    const volume = new MilkyWayProceduralVolume(11_400, 1_100);
    const texture = volume.material.uniforms['densityTexture']!.value as THREE.Data3DTexture;
    const densityData = texture.image.data;
    ...
    volume.mesh.geometry.addEventListener('dispose', () => {
      geometryDisposeCount += 1;
    });
    volume.material.addEventListener('dispose', () => {
    ...
    expect(volume.material.uniforms['integratedLightResolution']!.value).toBe(0.74);

    volume.update(0.82, 1.3, 2);
    expect(volume.material.uniforms['integratedLightResolution']!.value).toBe(1);
    ...
    const secondVolume = new MilkyWayProceduralVolume(11_400, 1_100);
    const secondTexture = secondVolume.material.uniforms['densityTexture']!
      .value as THREE.Data3DTexture;

    expect(secondTexture).not.toBe(texture);
    expect(secondTexture.image.data).toBe(densityData);
