...
const VOLUME_PROFILES = {
  low: { rayMarchSteps: 8, absorption: 3.8, dustAbsorption: 6.6, brightness: 1.55 },
  medium: { rayMarchSteps: 16, absorption: 4.3, dustAbsorption: 7.5, brightness: 1.95 },
  high: { rayMarchSteps: 32, absorption: 4.8, dustAbsorption: 8.6, brightness: 2.35 },
...
export class MilkyWayProceduralVolume {
  ...
  constructor(diameter: number, thickness: number) {
    this.material = createVolumeMaterial(
      this.densityTexture,
      VOLUME_PROFILES.medium,
      new THREE.Vector3(diameter / 2, thickness / 2, diameter / 2),
    );
    ...
    this.mesh.userData['viewAdaptiveTreatment'] =
      'crisp-exterior-arm-contrast-and-restrained-interior-integrated-light';
    this.mesh.userData['observerResolutionTreatment'] =
      'nearby-unresolved-density-resolves-spatially-into-co-spatial-batched-stars';
    this.mesh.userData['approachResolutionTreatment'] =
  ...
  public update(opacity: number, radiance: number, integratedLightResolution = 0): void {
    this.material.uniforms['volumeOpacity']!.value = opacity;
    this.material.uniforms['radiance']!.value = radiance;
    this.material.uniforms['integratedLightResolution']!.value = THREE.MathUtils.clamp(
...
function sampleDensity(target: MilkyWayDensitySample, x: number, y: number, z: number): void {
  ...
  for (let armIndex = 0; armIndex < 4; armIndex += 1) {
    ...
    armDensity +=
      armGate *
      (broadProfile * (majorArm ? 0.082 : 0.034) * (0.48 + cloudStrength * 0.52) +
        ridge * (majorArm ? 0.64 : 0.38) * fragmentedStrength +
        splitRidge * cloudStrength * (majorArm ? 0.31 : 0.18)) *
      armStrength;
    youngStarDensity +=
  ...
  const diffuseDisc =
    radialMask *
    (0.004 + interarmClouds * 0.028 + filamentNoise * 0.0035) *
    (0.68 + radialGlow * 0.4);
  const discDensity =
    diffuseDisc * (thinDisc + thickDisc) + armDensity * 1.18 * (thinDisc * 1.04 + thickDisc * 0.07);
...
function createVolumeMaterial(
  ...
  volumeHalfExtents: THREE.Vector3,
  ...
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      radiance: { value: 1 },
      integratedLightResolution: { value: 0 },
      volumeHalfExtents: { value: volumeHalfExtents },
    },
    vertexShader: `
    ...
    fragmentShader: `
      ...
      uniform float integratedLightResolution;
      ...
      void main() {
        ...
        float observerDistance = length(volumeRayOrigin);
        float exteriorView = smoothstep(1.02, 1.42, observerDistance);
        ...
        for (int index = 0; index < MAXIMUM_STEPS; index += 1) {
          ...
          float observerAuthoringDistance = length(
            (samplePosition - volumeRayOrigin) * volumeHalfExtents
          );
          float localIntegratedLightPresence = mix(
            1.0,
            smoothstep(180.0, 1050.0, observerAuthoringDistance),
            insideVolume
          );
          ...
          if (max(emissionDensity, dustDensity) > 0.006) {
            ...
            vec3 coolCloud = vec3(0.018, 0.048, 0.22);
            vec3 violetCloud = vec3(0.42, 0.045, 0.76);
            vec3 neutralStarlight = vec3(0.78, 0.56, 0.34);
            vec3 sapphireStars = vec3(0.045, 0.28, 1.4);
            vec3 ionizedCyan = vec3(0.028, 0.82, 1.18);
            vec3 pinkNebula = vec3(1.28, 0.045, 0.52);
            vec3 warmCore = vec3(1.42, 0.34, 0.035);
            vec3 ivoryNucleus = vec3(1.5, 1.02, 0.66);
            vec3 coolIntegratedLight = mix(
              coolCloud,
              violetCloud,
              smoothstep(0.48, 0.86, nebulaNoise) * (0.2 + youngStarTint * 0.28)
            );
            ...
            sampleColor = mix(sampleColor, pinkNebula, hiiStrength * 0.74);
            vec3 dustColor = mix(
              darkDust,
              warmDustEdge,
            ...
            float chromaGain = 1.18
              + youngArmStrength * 0.34
              + barStrength * 0.16
              + hiiStrength * 0.42;
            sampleColor = max(
              vec3(0.0),
            ...
            float unresolvedVolumePresence = mix(
              1.0,
              0.14,
              integratedLightResolution
            );
            ...
            sampleAlpha *= volumeOpacity
              ...
              * mix(0.98, 1.08, exteriorView)
              * localIntegratedLightPresence
              * unresolvedVolumePresence;
