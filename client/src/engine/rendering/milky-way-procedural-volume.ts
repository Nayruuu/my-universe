...
const VOLUME_PROFILES = {
  low: { rayMarchSteps: 8, absorption: 3.5, dustAbsorption: 5.2, brightness: 1.14 },
  medium: { rayMarchSteps: 16, absorption: 3.85, dustAbsorption: 6.1, brightness: 1.32 },
  high: { rayMarchSteps: 32, absorption: 4.2, dustAbsorption: 7, brightness: 1.5 },
...
function sampleDensity(target: MilkyWayDensitySample, x: number, y: number, z: number): void {
  ...
  for (let armIndex = 0; armIndex < 4; armIndex += 1) {
    ...
    const armDust = gaussianProfile(leadingDustDistance, localArmWidth * 0.22);

    dustLane = Math.max(
      dustLane,
      armGate * armDust * fragmentedStrength * (0.24 + filamentNoise * 0.28 + cloudStrength * 0.3),
    );
  }
  ...
  const thickDisc = Math.exp(-Math.abs(y) * 2.8) * 0.2;
  ...
  const diffuseDisc =
    radialMask *
    (0.018 + interarmClouds * 0.09 + filamentNoise * 0.012) *
    (0.68 + radialGlow * 0.4);
  const discDensity =
    diffuseDisc * (thinDisc + thickDisc) + armDensity * 0.68 * (thinDisc * 1.04 + thickDisc * 0.22);
  const centralDensity =
    (barDensity * 0.26 + boxyBulge * 0.2 + roundBulge * 0.22 + nucleusDensity * 0.42) *
    (0.76 + filamentNoise * 0.24) *
    centralEnvelope;
  const patchiness =
    0.34 +
    smoothstep(0.28, 0.76, broadNoise * 0.36 + cloudNoise * 0.4 + filamentNoise * 0.24) * 0.74;
  const midplaneDust =
    radialMask *
    Math.exp(-Math.abs(y) * 24) *
    (1 - smoothstep(0.72, 0.96, warpedRadius)) *
    (0.04 + filamentNoise * 0.1 + broadNoise * 0.04);
  const combinedDust = Math.max(dustLane, midplaneDust);
  const carvedDensity = (discDensity * patchiness + centralDensity) * (1 - combinedDust * 0.72);
  const smoothFineStructure =
    0.42 + filamentNoise * 0.22 + cloudNoise * 0.14 + broadNoise * 0.08 + cellNoise * 0.14;

  target.emission = THREE.MathUtils.clamp(carvedDensity * smoothFineStructure, 0, 1);
  target.dust = THREE.MathUtils.clamp(
    combinedDust *
      (thinDisc * 0.62 + thickDisc * 0.2 + centralEnvelope * 0.24) *
      (0.58 + filamentNoise * 0.42),
    0,
    1,
  );
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        for (int index = 0; index < MAXIMUM_STEPS; index += 1) {
          ...
          float emissionDensity = pow(densitySample.r, 1.18);
          float dustDensity = densitySample.g;
          float youngStars = densitySample.b;
          float warmth = densitySample.a;

          if (max(emissionDensity, dustDensity) > 0.008) {
            float luminousCloud = smoothstep(0.02, 0.24, emissionDensity);
            ...
            float youngStarTint = smoothstep(0.004, 0.13, youngStars);
            float warmTint = smoothstep(0.025, 0.48, warmth);
            float sparkle = pow(detailNoise, 42.0) * youngStarTint;
            float cloudGrain = 0.74 + smoothstep(0.18, 0.82, detailNoise) * 0.38;
            ...
            float nucleusGlow = pow(warmth, 4.0);
            vec3 darkDust = vec3(0.008, 0.006, 0.005);
            vec3 brownDust = vec3(0.07, 0.028, 0.012);
            vec3 coolCloud = vec3(0.045, 0.09, 0.2);
            vec3 neutralStarlight = vec3(0.52, 0.44, 0.34);
            vec3 blueStars = vec3(0.16, 0.4, 0.9);
            vec3 pinkNebula = vec3(0.88, 0.16, 0.42);
            vec3 warmCore = vec3(0.98, 0.5, 0.16);
            vec3 ivoryNucleus = vec3(1.1, 0.82, 0.5);
            vec3 sampleColor = mix(
              coolCloud,
              neutralStarlight,
              clamp(luminousCloud * 0.72 + warmTint * 0.14, 0.0, 1.0)
            );

            sampleColor = mix(
              sampleColor,
              blueStars,
              clamp(youngStarTint * 0.64 + sparkle * 0.34, 0.0, 0.74)
            );
            sampleColor = mix(sampleColor, pinkNebula, nebula * 0.38);
            sampleColor = mix(sampleColor, warmCore, warmTint * 0.76);
            vec3 dustColor = mix(
              darkDust,
              brownDust,
              clamp(warmth * 0.34 + dustRatio * 0.12, 0.0, 0.42)
            );
            sampleColor = mix(sampleColor, dustColor, clamp(dustRatio * 1.08, 0.0, 0.94));
            sampleColor *= cloudGrain;
            sampleColor += blueStars * youngStarTint * luminousCloud * 0.14;
            sampleColor += mix(blueStars, warmCore, warmth) * microStar * 1.7;
            sampleColor += ivoryNucleus * nucleusGlow * 0.24;
            ...
            float opticalDensity =
              (emissionDensity + stellarEmission * 0.34) * absorption
              + dustDensity * dustAbsorption;
