...
export class ConstellationVisual {
  ...
  constructor(
    private readonly catalog: ConstellationCatalog,
  ...
  public updatePositions(): void {
    const positionAttribute = this.lines.geometry.getAttribute('position') as THREE.BufferAttribute;
    const positions = positionAttribute.array as Float32Array;
    const from = new THREE.Vector3();
    const to = new THREE.Vector3();
    const boundPosition = new THREE.Vector3();
    let offset = 0;

    for (const figure of this.catalog.figures) {
      for (const [fromId, toId] of figure.segments) {
        requirePosition(this.registry, fromId, figure.id, from);
        requirePosition(this.registry, toId, figure.id, to);
    ...
    for (const figure of this.figuresById.values()) {
      updateFigureBounds(figure, this.registry, boundPosition);
      this.definitionsById.get(figure.objectId)!.visual.visualRadius = figure.radius;
    }
    positionAttribute.needsUpdate = true;
    this.lines.geometry.computeBoundingSphere();
    const activeObjectId = this.highlightLines.userData['objectId'];

    if (typeof activeObjectId === 'string') {
      this.showHighlight(activeObjectId);
    }
  }
...
function createFigureGeometry(
  ...
  return {
    ...
    catalogIds: [...uniquePositions.keys()],
    brightestMagnitude,
...
function updateFigureBounds(
  ...
  figure.center.set(0, 0, 0);
  for (const catalogId of figure.catalogIds) {
    requirePosition(registry, catalogId, figure.figure.id, position);
    figure.center.add(position);
  }
  figure.center.multiplyScalar(1 / figure.catalogIds.length);
  figure.radius = 0;
  for (const catalogId of figure.catalogIds) {
    requirePosition(registry, catalogId, figure.figure.id, position);
    figure.radius = Math.max(figure.radius, figure.center.distanceTo(position));
  }
  figure.radius = Math.max(figure.radius, 0.25);
}
