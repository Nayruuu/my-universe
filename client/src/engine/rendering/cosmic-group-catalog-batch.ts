...
export class CosmicGroupCatalogBatch {
  ...
  private refreshPointVisibility(): void {
    ...
    const nextVisibleDrawCount = this.layers.groups && this.opacity > 0.004 ? drawCount : 0;
    const visible = nextVisibleDrawCount > 0;

    // Reveal thresholds are sorted. Updating only the prefix that changed avoids clearing and
    // rebuilding the 37k-entry picking mask every animation frame while the catalogue is stable.
    // Hidden groups keep no selectable entries, exactly as before.
    if (nextVisibleDrawCount > this.visibleDrawCount) {
      this.visibleIndices.fill(1, this.visibleDrawCount, nextVisibleDrawCount);
    } else if (nextVisibleDrawCount < this.visibleDrawCount) {
      this.visibleIndices.fill(0, nextVisibleDrawCount, this.visibleDrawCount);
    }
    this.visibleDrawCount = nextVisibleDrawCount;
    this.activePointCount = nextVisibleDrawCount;
