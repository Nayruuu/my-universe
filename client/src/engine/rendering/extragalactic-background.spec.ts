...
describe('ExtragalacticBackground', () => {
  it('construit un fond de galaxies étendues en un seul lot explicitement illustratif', () => {
    ...
    expect(geometry.getAttribute('position').count).toBe(28_000);
    expect(geometry.getAttribute('galaxyColor').count).toBe(28_000);
    expect(geometry.getAttribute('galaxySize').count).toBe(28_000);
    expect(geometry.getAttribute('galaxyAlpha').count).toBe(28_000);
    expect(geometry.getAttribute('galaxyAngle').count).toBe(28_000);
    expect(geometry.getAttribute('galaxyAxisRatio').count).toBe(28_000);
    expect(geometry.getAttribute('galaxyProfile').count).toBe(28_000);
    expect(geometry.getAttribute('galaxyProminence').count).toBe(28_000);
    expect(geometry.getAttribute('galaxySeed').count).toBe(28_000);
    expect(geometry.getAttribute('galacticTransmission').count).toBe(28_000);
    expect(geometry.drawRange.count).toBe(14_000);
    ...
    for (const index of [0, 1_000, 14_000, 27_999]) {
      expect(
  ...
  it('adapte la densité et le ratio de pixels sans suivre la caméra', () => {
    const background = new ExtragalacticBackground();
    ...
    expect(background.points.geometry.drawRange.count).toBe(6_000);
    expect(background.points.material.uniforms['qualityScale']!.value).toBe(1.24);
    background.setQuality('high');
    expect(background.points.geometry.drawRange.count).toBe(28_000);
    ...
    expect(background.points.position.toArray()).toEqual([0, 0, 0]);
  ...
  it('apparaît et disparaît continûment entre les échelles locales et cosmologiques', () => {
    ...
    expect(calculateExtragalacticBackgroundOpacity(90_000)).toBeCloseTo(0.62, 8);
    expect(calculateExtragalacticBackgroundOpacity(120_000)).toBeCloseTo(0.31, 8);
    expect(calculateExtragalacticBackgroundOpacity(150_000)).toBe(0);
    ...
    for (const boundary of [1_600, 3_200, 90_000, 150_000]) {
