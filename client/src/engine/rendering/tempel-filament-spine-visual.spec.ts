...
describe('TempelFilamentSpineVisual', () => {
  ...
  it('réutilise les limites précalculées sans rescanner les halos sur le thread principal', () => {
    const source = createSpineCatalog();
    const catalog: TempelFilamentSpineCatalog = {
      ...source,
      renderData: prepareTempelFilamentSpineRenderData(source, 200),
    };
    const computeBounds = vi.spyOn(LineSegmentsGeometry.prototype, 'computeBoundingBox');
    const computeSphere = vi.spyOn(LineSegmentsGeometry.prototype, 'computeBoundingSphere');
    const visual = createTempelFilamentSpineVisual(catalog, createRegistry(), 200);

    expect(computeBounds).not.toHaveBeenCalled();
    expect(computeSphere).not.toHaveBeenCalled();
    expect(visual.haloTiles[0]!.geometry.boundingBox).not.toBeNull();
    expect(visual.haloTiles[0]!.geometry.boundingSphere?.radius).toBeGreaterThan(0);
  });
});
