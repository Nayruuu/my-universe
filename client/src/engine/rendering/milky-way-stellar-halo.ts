...
// Keep the compact globular-like cues sparse: the extended envelope is primarily a field of
// isolated stars, rather than a sequence of decorative clusters during the approach.
const STELLAR_HALO_CLUSTER_PARTICLE_PERIOD = 16;
...
// About 129 kpc from the Galactic centre in the canonical inner-Galactic metric. This is an
// illustrative sparse stellar envelope around the documented 100,000 light-year luminous disc,
// not a rescaling of that disc or of the Solar position inside it.
const STELLAR_HALO_OUTER_RADIUS = MILKY_WAY_PROCEDURAL_AUTHORING_DIAMETER * 4.2;
// This outer population is much rounder than the luminous disc. It must retain stellar context
// above and below the plane too, instead of becoming empty on a vertical approach.
const STELLAR_HALO_VERTICAL_FLATTENING = 0.9;
 ...
 * disc. Its deliberately wide, sparse envelope makes the approach materially traversable before
 * the denser disc grains resolve. It is not a diffuse glow: black remains visible between stars,
 * and the compact globular-like concentrations are visual cues rather than catalogued clusters.
 */
export class MilkyWayStellarHalo {
  ...
  constructor() {
    ...
    this.points.userData['outerEnvelopeTreatment'] =
      'illustrative-extended-stellar-halo-surrounding-the-documented-luminous-disc';
  ...
  public update(targetOpacity: number, deltaSeconds: number, sceneScale: MilkyWaySceneScale): void {
    ...
    this.points.userData['physicalOuterRadiusKiloparsecs'] =
      this.points.userData['worldOuterRadius'] / sceneScale.visualSceneUnitsPerKiloparsec;
...
function sampleIllustrativeHaloRadius(random: () => number): number {
  // A broad outer component ensures the approach reads as a gradually thickening stellar volume,
  // instead of revealing all of its stars only when the luminous disc is already nearby.
  if (random() < 0.58) {
    return THREE.MathUtils.lerp(
      STELLAR_HALO_OUTER_RADIUS * 0.54,
