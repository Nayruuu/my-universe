...
import { type LocalMilkyWayPanoramaStatus } from './local-space-environment';
import { type MilkyWayAtlasStatus } from './milky-way-volume';
...
import {
  ...
  type RenderingPrewarmTarget,
  type TexturePrewarmTarget,
} from './texture-prewarm-target';

const INITIAL_PANORAMA_PREWARM_BUDGET_MS = 500;
...
export class UniverseScene {
  ...
  private readonly environment: UniverseSceneEnvironment;
  ...
  constructor(private readonly performanceManager: PerformanceManager) {
    ...
    this.environment = new UniverseSceneEnvironment(
      this.scene,
      this.spaceRoot,
  ...
  public get milkyWayAtlasStatus(): MilkyWayAtlasStatus {
    return this.environment.milkyWayAtlasStatus;
  }

  public get milkyWayVolumeDrawMeshCount(): number {
    return this.environment.milkyWayVolumeDrawMeshCount;
  }

  public get localMilkyWayPanoramaStatus(): LocalMilkyWayPanoramaStatus {
    return this.environment.localMilkyWayPanoramaStatus;
  }

  public async ensureMilkyWayAtlas(): Promise<boolean> {
    return this.environment.ensureMilkyWayAtlas();
  }

  public async prewarmMilkyWayAssets(target: TexturePrewarmTarget): Promise<boolean> {
    return this.environment.prewarmMilkyWayAssets(target);
  }

  public async prewarmRendering(
    target: RenderingPrewarmTarget,
    camera: THREE.Camera,
  ): Promise<boolean> {
    const texturesReady = await this.prewarmMilkyWayAssets(target);
    const materialsReady = await prewarmSceneMaterials(target, this.scene, camera);

    return texturesReady && materialsReady;
  }

  public async prewarmSceneMaterials(
    target: RenderingPrewarmTarget,
    camera: THREE.Camera,
  ): Promise<boolean> {
    return prewarmSceneMaterials(target, this.scene, camera);
  }

  public async prewarmInitialRendering(
    target: RenderingPrewarmTarget,
    camera: THREE.Camera,
  ): Promise<boolean> {
    // Give the optional panorama a bounded head start alongside shader compilation, so its
    // asynchronous decode does not normally shift the upload onto the first navigation frame.
    let timeoutId!: ReturnType<typeof setTimeout>;
    const panoramaDeadline = new Promise<void>((resolve) => {
      timeoutId = setTimeout(resolve, INITIAL_PANORAMA_PREWARM_BUDGET_MS);
    });
    const panoramaReady = Promise.race([this.prewarmMilkyWayAssets(target), panoramaDeadline]);
    const materialsReady = await prewarmSceneMaterials(target, this.scene, camera);
  ...
  public setPixelRatio(pixelRatio: number): void {
    this.environment.setPixelRatio(pixelRatio);
  ...
  public async setTempelFilamentSpineCatalog(
    ...
    return this.cosmicCatalogLayers.setTempelFilamentSpineCatalog(
      catalog,
      registry,
      coordinateSystem,
    );
  ...
  public async prepareTempelFilamentSpineCatalog(
    ...
    return this.cosmicCatalogLayers.prepareTempelFilamentSpineCatalog(
      catalog,
      registry,
      coordinateSystem,
    );
  ...
  public updateStellarCatalog(time: UniverseTime, temporalMode: TemporalMode = 'state'): void {
    this.stellarCatalogLayers.updateTime(time, temporalMode);
  }
