...
import {
  ...
  calculateGalacticDustCoarseDetailPresence,
  calculateGalacticDustTraversalOpacity,
  calculateGalacticImmersionDetailOpacity,
  calculateGalacticMacroscopicDetailOpacity,
  calculateMilkyWayParticleDensityCompensation,
  calculateMilkyWayParticleDrawCount,
...
describe('GalacticTransitionLayer', () => {
  it('installe un fond stellaire procédural et une galaxie de points centrée', () => {
    ...
    expect(extragalacticBackground.userData).toMatchObject({
      ...
      observerAnchoring: 'galactocentric-static-shell',
      motionModel: 'fixed-galactocentric-points-with-perspective-parallax',
    });
    expect(extragalacticBackground.geometry.getAttribute('position').count).toBe(28_000);
    expect(extragalacticBackground.geometry.drawRange.count).toBe(14_000);
  ...
  it('garde le fond autour de la caméra après un recentrage d’origine flottante', () => {
    ...
    expect(extragalacticWorldPosition.toArray()).toEqual(spaceRoot.position.toArray());
  ...
  it('adapte les densités et le ratio de pixels à la qualité', () => {
    ...
    expect(extragalacticBackground.geometry.drawRange.count).toBe(6_000);
    expect(extragalacticBackground.material.uniforms['qualityScale']!.value).toBe(1.24);
    ...
    expect(stellarHalo.geometry.drawRange.count).toBe(16_000);
    ...
    expect(backdrop.geometry.drawRange.count).toBe(7_000);
    expect(extragalacticBackground.geometry.drawRange.count).toBe(14_000);
    expect(extragalacticBackground.material.uniforms['qualityScale']!.value).toBe(1.1);
    ...
    expect(stellarHalo.geometry.drawRange.count).toBe(32_000);
    ...
    expect(extragalacticBackground.geometry.drawRange.count).toBe(28_000);
    ...
    expect(stellarHalo.geometry.drawRange.count).toBe(64_000);
  ...
  it('réduit uniquement le budget de particules sub-pixel de la Voie lactée lointaine', () => {
    const { layer, spaceRoot } = createLayer();
    const milkyWay = getMilkyWay(spaceRoot);

    expect(calculateMilkyWayParticleDrawCount('high', 36_000)).toBe(280_000);
    expect(calculateMilkyWayParticleDrawCount('high', 36_001)).toBe(140_000);
    expect(calculateMilkyWayParticleDrawCount('high', 96_001)).toBe(70_000);
    expect(calculateMilkyWayParticleDensityCompensation('high', 280_000)).toBe(1);
    expect(calculateMilkyWayParticleDensityCompensation('high', 140_000)).toBeCloseTo(Math.sqrt(2));
    expect(calculateMilkyWayParticleDensityCompensation('high', 70_000)).toBe(2);

    layer.setQuality('high');
    layer.update({
      lodLevel: 4,
      deltaSeconds: 1,
      cameraDistance: 96_001,
      starRadiance: 1,
      galaxyRadiance: 1,
    });

    expect(milkyWay.geometry.drawRange.count).toBe(70_000);
    expect(milkyWay.material.uniforms['qualityDensityCompensation']!.value).toBe(2);
    expect(milkyWay.userData).toMatchObject({
      activeParticleCount: 70_000,
      particleBudgetTreatment:
  ...
  it('ne change ni les étoiles ni leur luminosité intrinsèque pendant la traversée', () => {
    ...
    for (const distance of [14_000, 9_000, 5_000, 3_600, 2_200, 1_400, 520]) {
      ...
      expect(milkyWay.material.uniforms['opacity']!.value).toBeCloseTo(
        calculateGalacticMacroscopicDetailOpacity(distance),
        8,
      );
    }
    layer.dispose();
  ...
  it('conserve le volume procédural indépendamment de l’atlas et le masque hors échelle', () => {
    ...
    expect(fallbackOpacity).toBeCloseTo(calculateGalacticMacroscopicDetailOpacity(3_600), 6);
  ...
  it('superpose les couches galactiques au catalogue local sans rupture de LOD', () => {
    ...
    expect(milkyWay.visible).toBe(true);
    expect(milkyWay.material.uniforms['opacity']!.value).toBeCloseTo(
      calculateGalacticMacroscopicDetailOpacity(1_400),
      6,
    );
  ...
  it('résout la silhouette en poussière fixe avant le relais HYG/Gaia', () => {
    expect(calculateGalacticMacroscopicDetailOpacity(6_000)).toBeCloseTo(0.96, 8);
    expect(calculateGalacticMacroscopicDetailOpacity(2_200)).toBeGreaterThan(0.44);
    expect(calculateGalacticMacroscopicDetailOpacity(2_200)).toBeLessThan(0.46);
    expect(calculateGalacticMacroscopicDetailOpacity(1_400)).toBeGreaterThan(0.33);
    expect(calculateGalacticMacroscopicDetailOpacity(1_400)).toBeLessThan(0.35);
    expect(calculateGalacticMacroscopicDetailOpacity(600)).toBeLessThan(0.35);
    expect(calculateGalacticMacroscopicDetailOpacity(600)).toBeGreaterThan(0.25);
    expect(calculateGalacticDustTraversalOpacity(600)).toBeCloseTo(0.96, 8);
    expect(calculateGalacticDustCoarseDetailPresence(5_400)).toBe(1);
    expect(calculateGalacticDustCoarseDetailPresence(2_000)).toBeLessThan(0.25);
    expect(calculateGalacticDustCoarseDetailPresence(1_200)).toBe(0);
    expect(calculateGalacticDustCoarseDetailPresence(Number.NaN)).toBe(0);
    expect(calculateGalacticDustTraversalOpacity(240)).toBeGreaterThan(
      calculateGalacticMacroscopicDetailOpacity(240) * 3,
    );
    expect(calculateGalacticDustTraversalOpacity(90)).toBeGreaterThan(0);
    expect(calculateGalacticDustTraversalOpacity(Number.NaN)).toBe(0);
  });
