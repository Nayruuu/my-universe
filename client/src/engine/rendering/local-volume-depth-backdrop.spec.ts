...
describe('LocalVolumeDepthBackdrop', () => {
  ...
  it('préserve les directions Cosmicflows tout en comprimant la profondeur du LOD', () => {
    ...
    expect(projected.x).toBeCloseTo(13_200, 8);
    expect(projected.y).toBeCloseTo(17_600, 8);
  ...
  it('conserve un champ profond continu entre la Voie lactée et l’Univers proche', () => {
    expect(getLocalVolumeDepthBackdropOpacity(4_800)).toBe(0);
    expect(getLocalVolumeDepthBackdropOpacity(7_500)).toBeGreaterThan(0);
    expect(getLocalVolumeDepthBackdropOpacity(12_000)).toBeCloseTo(0.46, 5);
    expect(getLocalVolumeDepthBackdropOpacity(17_000)).toBeCloseTo(0.46, 5);
    expect(getLocalVolumeDepthBackdropOpacity(60_000)).toBeCloseTo(0.46, 5);
    expect(getLocalVolumeDepthBackdropOpacity(120_000)).toBeGreaterThan(0);
    expect(getLocalVolumeDepthBackdropOpacity(150_000)).toBe(0);

    const before = getLocalVolumeDepthBackdropOpacity(94_999);
    const after = getLocalVolumeDepthBackdropOpacity(95_001);
  ...
  it('pilote qualité, radiance et visibilité sans rendre le fond interactif', () => {
    ...
    expect(backdrop.points.material.uniforms['opacity']!.value).toBeCloseTo(0.46, 5);
    ...
    backdrop.updateDistance(150_000, 10);
    expect(backdrop.points.visible).toBe(false);
    expect(backdrop.visibleCount).toBe(0);
    backdrop.dispose();
  });
