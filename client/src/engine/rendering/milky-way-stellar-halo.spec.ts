...
describe('MilkyWayStellarHalo', () => {
  it('construit un environnement stellaire étendu, discontinu et explicitement illustratif', () => {
    ...
    expect(points.userData).toMatchObject({
      ...
      clusterCount: 48,
      clusterParticleFraction: 1 / 16,
      verticalFlattening: 0.9,
      quality: 'medium',
      qualityDensityCompensation: 1.4,
    });
    expect(points.userData['authoringInnerRadius']).toBeCloseTo(3_876, 8);
    expect(points.userData['authoringOuterRadius']).toBeCloseTo(47_880, 8);
    expect(positions.count).toBe(64_000);
    expect(points.geometry.getAttribute('color').count).toBe(64_000);
    expect(points.geometry.getAttribute('pointSize').count).toBe(64_000);
    expect(points.geometry.getAttribute('pointAlpha').count).toBe(64_000);
    expect(clusterMembership.count).toBe(64_000);
    expect(Array.from(clusterMembership.array).filter((value) => value === 1)).toHaveLength(4_000);
    expect(points.geometry.drawRange.count).toBe(32_000);
    expect(countOutsideDiscRadius(positions, 5_700)).toBeGreaterThan(53_000);
  ...
  it('adapte la densité et reste fixe dans le même référentiel que le disque', () => {
    ...
    halo.setQuality('low');
    expect(halo.points.geometry.drawRange.count).toBe(16_000);
    expect(halo.points.material.uniforms['qualityDensityCompensation']!.value).toBe(2);
    halo.setQuality('high');
    expect(halo.points.geometry.drawRange.count).toBe(64_000);
    ...
    expect(halo.points.userData['worldOuterRadius']).toBeCloseTo(47_880 * sceneScale.modelScale, 8);
    expect(halo.points.userData['physicalOuterRadiusKiloparsecs']).toBeCloseTo(128.77, 2);
    // A target-centred approach from 9,000 world units stays inside this actual point envelope
    // even from the far side of the Sun. This is geometry coverage, not a camera-scale trick.
    expect(halo.points.userData['worldOuterRadius']).toBeGreaterThan(9_000 + 736.02);
