...
const EXTRAGALACTIC_OUTER_FADE_START = 90_000;
const EXTRAGALACTIC_OUTER_FADE_END = 150_000;
...
/**
 * A galactocentric, non-interactive sample of the much larger cosmological galaxy population. The
 * positions and appearances are procedural visual context, not a catalogue or a one-to-one model
 * of observable galaxies. One fixed point batch supplies extended impostors without per-galaxy
 * objects, so an approach reads perspective parallax rather than a camera-following sky swap.
 */
export class ExtragalacticBackground {
  ...
  constructor() {
    ...
    this.points.userData['observerAnchoring'] = 'galactocentric-static-shell';
    this.points.userData['motionModel'] = 'fixed-galactocentric-points-with-perspective-parallax';
