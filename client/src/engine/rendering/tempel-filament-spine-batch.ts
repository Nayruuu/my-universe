...
export function updateTempelFilamentSelectionMask(
  visibleIndices: Uint8Array,
  previousSegmentCount: number,
  nextSegmentCount: number,
): void {
  if (nextSegmentCount > previousSegmentCount) {
    visibleIndices.fill(1, previousSegmentCount * 2, nextSegmentCount * 2);
  } else if (nextSegmentCount < previousSegmentCount) {
    visibleIndices.fill(0, nextSegmentCount * 2, previousSegmentCount * 2);
  }
}
