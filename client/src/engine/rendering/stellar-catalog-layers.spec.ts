...
describe('StellarCatalogLayers', () => {
  it('expose un état vide sûr avant le chargement des catalogues', () => {
    ...
    expect(layers.hasStarClusterObject('unknown')).toBe(false);
    expect(layers.getStarClusterDefinition('unknown')).toBeUndefined();
  ...
  it('remplace le catalogue stellaire et réinitialise ses amas sans fuite GPU', async () => {
    ...
    expect(layers.getGaiaPresentationStats(new THREE.PerspectiveCamera()).sampledSources).toBe(1);
    expect(layers.getCatalogWorldPosition('hyg-3229')).toBeInstanceOf(THREE.Vector3);
    expect(layers.getPickables()).toHaveLength(3);
    expect(layers.hasStarClusterObject('gaia-dr3-source-123456789')).toBe(true);
    expect(layers.getStarClusterDefinition('gaia-dr3-source-123456789')).toMatchObject({
      id: 'gaia-dr3-source-123456789',
      name: 'Gaia DR3 123456789',
      type: 'star',
    });
    expect(
      layers.getStarClusterDefinition('gaia-dr3-source-123456789')?.metadata?.['colorIndexBpRp'],
    ).toBeCloseTo(0.2);
    expect(layers.getCatalogWorldPosition('gaia-dr3-source-123456789')).toBeInstanceOf(
      THREE.Vector3,
    );
    ...
    expect(firstPoints.userData['fieldRendering']).toBe('rendered');
    expect(firstPoints.visible).toBe(true);
