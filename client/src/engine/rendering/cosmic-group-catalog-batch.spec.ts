...
describe('CosmicGroupCatalogBatch', () => {
  ...
  it('ne réécrit pas le masque Cosmicflows stable ou caché à chaque image', () => {
    const batch = createBatch();
    ...
    batch.updateDistance(420_000, 10);
    fill.mockClear();
    batch.updateDistance(420_000, 1 / 60);
    expect(fill).not.toHaveBeenCalled();

    batch.updateDistance(40_000, 10);
    expect(visibleIndices).toEqual(new Uint8Array([0, 0]));
    expect(fill).toHaveBeenCalledExactlyOnceWith(0, 0, 1);
    batch.dispose();
  });
