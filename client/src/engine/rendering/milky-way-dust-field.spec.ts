...
describe('MilkyWayDustField', () => {
  it('dépose la densité de la galaxie affichée, sans inventer un catalogue ou un ciel uniforme', () => {
    ...
    expect(data[center + 64]).toBe(232);
    expect(data[center + 2_048]).toBe(232);
    ...
    expect(material.vertexShader).toContain('5.0 * pixelRatio');
  ...
  it('borne le dépôt aux cellules valides et conserve les trous du modèle source', () => {
    ...
    expect(data[1 + 64 * (1 + 32)]).toBe(23);
    expect(data[62 + 64 * (30 + 32 * 62)]).toBe(23);
    expect(data[32 + 64 * (16 + 32 * 32)]).toBe(0);
