...
- A point-built, explicitly illustrative Milky Way. One deterministic GPU batch draws
  144,000/168,000/280,000 samples of the same barred spiral and thick disc. These stars define the
  ...
  A separate static 16,000/32,000/64,000-point halo supplies sparse surroundings and 48 illustrative
  globular-like concentrations without fog. Its wide, rounder envelope reaches about 129 kpc from
  the Galactic centre as explicitly illustrative, uncatalogued context. A fixed galactocentric
  6,000/14,000/28,000-point extragalactic batch provides representative galaxy silhouettes with
  perspective parallax, while an external-only deep-field background remains separate.
...
- A seventh cosmic-web scale containing 37,730 calculated Cosmicflows-4 galaxy groups from 11.1 to
  ...
  non-interactive deep-sky batch introduces about 3,800/9,100/18,100 calculated groups in
...
The [archived clean campaign](docs/performance/2026-09-11-796fe70/campaign.json) on
`796fe70310b28ee17c1a0cd9445688d65f05b36c` passes **10/10** protocol/profile budgets, with three
runs or resource cycles each. Its ten unmodified reports are retained alongside the manifest;
their SHA-256 digests are verified by the documentation tests.

| Metric                               | Medium / CPU 4× | Low / CPU 6×   |
| ------------------------------------ | --------------- | -------------- |
| First usable map, median             | 1.47 s          | 2.05 s         |
| Tempel first frame, median / worst   | 27.2 / 29.4 ms  | 29.9 / 31.3 ms |
| Cold scale journey, worst frame      | 100.0 ms        | 66.7 ms        |
| Geometry / texture / draw-call drift | 0 / 0 / 0       | 0 / 0 / 0      |
| Observable Jupiter resolved          | 3/3             | 3/3            |

This is a same-host CPU stress baseline on Chrome 153 / M5 Max Metal, not representative
medium/low hardware. Medium reaches the 100 ms maximum budget, so first-use GPU work and outliers
remain under observation. The earlier isolated 78.2 ms Tempel spike was not reproduced in 20
instrumented follow-up runs; its cause was not established and no speculative rendering fix was made.

Dependency security fixes subsequently shipped in `c380bce26c21f92d89d4af55f86d74610f194e99`.
On 11 September, `npm ci`, `npm audit` (zero vulnerabilities), and `npm run verify` passed,
including 180 browser tests and all four coverage metrics at 100%. Only package metadata and the
lockfile changed. On 14 September, GitHub's Dependabot API returned no open alerts for this
repository. The performance campaign predates these dependency changes: rerun it on a clean revision
before claiming a newer performance baseline. The next product priority is visual approval of
arms/core/halo and traversal, without changing the camera.

### Current physical evidence: 14 September 2026

A clean physical desktop/high campaign at `5ef4fe39d660dd564255c01032852301a82f1c0c` ran all five
protocols on Chrome 153 with the Apple M5 Max Metal renderer. It passed **5/5** protocol budgets:
the first usable map median was 505.6 ms (524.6 ms worst); the Tempel first-visible frame was 0.7 ms
median and 0.8 ms worst after 3/3 successful invisible preloads; the three resource cycles retained
84 geometries, 16 textures, and 40 draw calls; three cold scale journeys remained within their global
budget; and three Retina planetarium journeys resolved Jupiter with a 16.8 ms worst frame and no long
frames. The batch is prepared and compiled before its normal reveal LOD, then reused unchanged at
activation. This is representative physical evidence for that high-end host only, not a claim about
medium or low hardware. Camera paths, targets, timing, and visual reveal ranges are unchanged.

### Historical measurements

The following records preserve intermediate diagnostics, not the current backlog. Their open-work
statements describe the state at measurement time; the clean 11 September campaign above supersedes
the pending-campaign status, without proving that every individual phase spike has disappeared.

A clean same-host simulated campaign recorded on 28 August 2026 at revision `27db0e1` passed all ten
...
- 2,950 unit and integration tests plus 169 static-data, documentation, deployment, benchmark, and
